...
#define HAPTIC_HV_DRIVER_VERSION	"v0.7.0.3"
#define AW_WAKELOCK

char *aw_ram_name = "aw86927_haptic_ram.bin";
char aw_rtp_name[][AW_RTP_NAME_MAX] = {
	{"aw86927_haptic_rtp_osc_24K_5s.bin"},
	{"aw86927_rtp_3.5_seconds.bin"},
	{"aw86927_honor_fisrt_kill_RTP_60.bin"},
	{"aw86927_honor_five_kill_RTP_64.bin"},
	{"aw86927_honor_four_continu_kill_RTP_66.bin"},
	{"aw86927_honor_four_kill_RTP_63.bin"},
	{"aw86927_honor_lengendary_RTP_69.bin"},
	{"aw86927_honor_thousands_kill_RTP_68.bin"},
	{"aw86927_honor_three_continu_kill_RTP_65.bin"},
	{"aw869274_honor_three_kill_RTP_62.bin"},
	{"aw86927_honor_two_kill_RTP_61.bin"},
	{"aw86927_honor_unstoppable_RTP_67.bin"},
	{"aw86927_alarm1_RTP.bin"},   //12
	{"aw86927_alarm2_RTP.bin"},
	{"aw86927_alarm3_RTP.bin"},
	{"aw86927_msg_long1_RTP.bin"},
	{"aw86927_msg_long2_RTP.bin"},
	{"aw86927_msg_short1_RTP.bin"},
	{"aw86927_msg_short2_RTP.bin"},  //18
	{"aw86927_msg_short3_RTP.bin"},
	{"aw86927_msg_short4_RTP.bin"},
	{"aw86927_msg_short5_RTP.bin"},
	{"aw86927_msg_short6_RTP.bin"},
	{"aw86927_msg_short7_RTP.bin"},
	{"aw86927_msg_one_RTP.bin"},   //24
	{"aw86927_msg_clasic_RTP.bin"},
	{"aw86927_msg_delight_RTP.bin"},
	{"aw86927_msg_health1_RTP.bin"},
	{"aw86927_msg_health2_RTP.bin"},
	{"aw86927_alarm4_RTP.bin"}     //29
};
...
static struct aw_haptic *m_aw_haptic;

extern bool pm_suspend_deepsleep(void);
			 ...
			 struct device_node *np)
{
	...
	aw_haptic->select_gpio = of_get_named_gpio(np, "select-gpio", 0);
	if (aw_haptic->select_gpio < 0) {
		aw_err("no select gpio provide");
		return -EPERM;
	}
...
static int judge_cali_range(struct aw_haptic *aw_haptic)
{
	...
	if (aw_haptic->f0 < f0_cali_min) {
		aw_err("f0 calibration out of min range = %d!", aw_haptic->f0);
		aw_haptic->f0_cali_data = 32;
		return -ERANGE;
	}

	if (aw_haptic->f0 > f0_cali_max) {
		aw_err("f0 calibration out of max range = %d!", aw_haptic->f0);
		aw_haptic->f0_cali_data = 31;
		return -ERANGE;
	}
...
static int ram_f0_cali(struct aw_haptic *aw_haptic)
{
	...
	while (cali_cnt > 0) {
		if (aw_haptic->func->ram_get_f0(aw_haptic)) {
			...
			if (cali_cnt == 2) /* save the motor reality F0 before the first calibration */
				aw_haptic->real_f0 = aw_haptic->f0;
			ret = judge_cali_range(aw_haptic);
			if (ret < 0)
				return -ERANGE;
			calculate_cali_data(aw_haptic);
			if (last_cali_data) {
				if (aw_haptic->f0_cali_data >= 32) {
					temp = last_cali_data - (64 - aw_haptic->f0_cali_data);
					if(temp <= 31 && temp >= 1 && last_cali_data >= 32)
						temp = 32;
					if(temp < 0)
						temp = aw_haptic->f0_cali_data + last_cali_data;
				} else if (aw_haptic->f0_cali_data <= 31 && aw_haptic->f0_cali_data > 0) {
					temp = (last_cali_data + aw_haptic->f0_cali_data) & 0x3f;
					if(temp >= 32 && last_cali_data <= 31)
						temp = 31;
				} else {
					temp = last_cali_data;
				}
				aw_haptic->f0_cali_data = temp;
			}
			aw_haptic->func->upload_lra(aw_haptic, AW_F0_CALI_LRA);
			last_cali_data = aw_haptic->f0_cali_data;
		}
		cali_cnt--;
		usleep_range(500000, 500500);
	}
	/* restore standby work mode */
	aw_haptic->func->play_stop(aw_haptic);
	return ret;
			       ...
			       struct ff_effect *old)
{
	...
	case FF_CONSTANT:
		...
		aw_haptic->index = AW_EFFECT_ID_BOUNDS;
	...
	case FF_PERIODIC:
		...
		if (wav_id > 0 && wav_id < AW_EFFECT_ID_BOUNDS) {
			aw_haptic->activate_mode = AW_RAM_MODE;
			aw_haptic->index = wav_id;
		} else if (wav_id > AW_EFFECT_ID_BOUNDS && wav_id <= wav_id_max) {
			aw_haptic->activate_mode = AW_RTP_MODE;
			aw_haptic->rtp_file_num = wav_id - AW_EFFECT_ID_BOUNDS;
		} else {
		...
		if (aw_haptic->activate_mode == AW_RAM_MODE) {
			...
			hrtimer_start(&aw_haptic->timer,
				      ktime_set(aw_haptic->duration / 1000,
						...
						1000000), HRTIMER_MODE_REL);
#ifdef AW_WAKELOCK
			__pm_stay_awake(&aw_haptic->ws_ram);
#endif
...
static int wait_enter_rtp_mode(struct aw_haptic *aw_haptic)
{
	...
	while (cnt) {
		ret = aw_haptic->func->juge_rtp_going(aw_haptic);
		if (ret) {
			rtp_work_flag = true;
			aw_info("RTP_GO!");
			break;
...
static void rtp_work_routine(struct work_struct *work)
{
	...
	pm_qos_enable(aw_haptic, true);
#ifdef AW_WAKELOCK
	__pm_stay_awake(&aw_haptic->ws_rtp);
#endif
	aw_haptic->rtp_cnt = 0;
	rtp_play(aw_haptic);
		       ...
		       char *buf)
{
	...
	len += snprintf(buf+len, PAGE_SIZE-len, "%d,%d\n",
			aw_haptic->real_f0, aw_haptic->f0_cali_data);
#endif
              ...
              const char *buf, size_t count)
{
    ...
    if (sscanf(buf, "%d %d", &f0_cali_data, &osc_cali_data) != 2) {
        aw_err("%s: err f0 :%d, osc :%d",
				__func__,f0_cali_data, osc_cali_data);
        return -1;
    }
    osc_cali_data = 0;
    if (f0_cali_data < 0 || f0_cali_data > 63) {
		aw_err("%s: err f0_cali:%d",
			__func__,f0_cali_data);
        return -2;
    }
    aw_info("%s: f0 :%d, osc :%d",
			__func__, f0_cali_data, osc_cali_data);
    aw_haptic->f0_cali_data = f0_cali_data;
    aw_haptic->osc_cali_data = osc_cali_data;
...
static void aw_force_stop(void)
{
	unsigned char reg_val = 0;
	/* 0x47 glb state
		bit3-bit0 0000 standby, 0111 ram-go, 1000 rtp-go, 1001 trig-go, 0110 cont-go
	*/
	haptic_hv_i2c_reads(m_aw_haptic, AW8692X_REG_GLBRD5,
					&reg_val, AW_I2C_BYTE_ONE);
	if((reg_val & 0x0f) != 0) {
		aw_info("%s force stop\n", __func__);
		m_aw_haptic->func->play_stop(m_aw_haptic);
	}
}

static int aw_stop_notify_call(struct notifier_block *nb, unsigned long event, void *data)
{
...
static int aw_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	gpio_direction_output(aw_haptic->select_gpio, 1);
	...
	m_aw_haptic = aw_haptic;
	aw_haptic->motor_nb.notifier_call = aw_stop_notify_call;
	ret = oplus_switch_notifier_register(SWITCH_MT, aw_haptic->dev, &aw_haptic->motor_nb);

#ifdef AW_WAKELOCK
	aw_haptic->ws_rtp.name = "haptic_rtp_wk_src";
	wakeup_source_add(&aw_haptic->ws_rtp);
	aw_haptic->ws_ram.name = "haptic_ram_wk_src";
	wakeup_source_add(&aw_haptic->ws_ram);
#endif
...
static int aw_remove(struct i2c_client *i2c)
{
	...
	free_pages((unsigned int)aw_haptic->start_buf, AW_TIKTAP_MMAP_PAGE_ORDER);
...
static int aw_i2c_suspend(struct device *dev)
{
	...
	if (pm_suspend_deepsleep()) {
		disable_irq(gpio_to_irq(m_aw_haptic->irq_gpio));
	}
#endif
...
#ifdef CONFIG_DEEPSLEEP
	if (pm_suspend_deepsleep()) {
		enable_irq(gpio_to_irq(m_aw_haptic->irq_gpio));
	}
#endif
