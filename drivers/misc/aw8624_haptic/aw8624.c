...
#define AW8624_DRIVER_VERSION "v1.3.12.1"
...
static const char aw8624_rtp_name[][AW8624_RTP_NAME_MAX] = {
    ...
    "aw8624_honor_fisrt_kill_RTP_60.bin",
    "aw8624_honor_five_kill_RTP_64.bin",
    "aw8624_honor_four_continu_kill_RTP_66.bin",
    "aw8624_honor_four_kill_RTP_63.bin",
    "aw8624_honor_lengendary_RTP_69.bin",
    "aw8624_honor_thousands_kill_RTP_68.bin",
    "aw8624_honor_three_continu_kill_RTP_65.bin",
    "aw8624_honor_three_kill_RTP_62.bin",
    "aw8624_honor_two_kill_RTP_61.bin",
    "aw8624_honor_unstoppable_RTP_67.bin",
};
...
static int aw8624_stop_notify_call(struct notifier_block *nb, unsigned long event, void *data);
...
static int aw8624_haptic_read_f0(struct aw8624 *aw8624)
{
	...
	if (!f0_reg) {
		aw8624->f0 = 0;
		pr_info("%s : get f0 failed with the value becoming 0!\n",
								__func__);
		return 0;
	}
...
static int aw8624_haptic_ram_config(struct aw8624 *aw8624)
{
	...
	if (aw8624->singlecycle != 0) {
		aw8624->singlecycle = 0;
		return 0;
	}

	if ((aw8624->duration > 0) && (aw8624->duration <
...
static void aw8624_rtp_work_routine(struct work_struct *work)
{
	...
	ret = pinctrl_select_state(aw8624->pinctrl, aw8624->pinstate);
	if (ret) {
		pr_err("%s: select state failed!\n", __func__);
	}
	...
	/* 20201125 add, to updata f0 when vibrate in ram&rtp mode */
...
static void aw8624_haptic_upload_lra(struct aw8624 *aw8624, unsigned int flag)
{
	...
	case AW8624_HAPTIC_ZERO:
		printk("%s write zero to trim_lra!\n", __func__);
...
static int aw8624_haptic_get_f0(struct aw8624 *aw8624)
{
	...
	f0_repeat_num = ((aw8624_dts_data.aw8624_parameter1 * aw8624->f0_pre
			* 100) / 5000 - 100 * (f0_pre_num + f0_wait_num + 2)) /
			(f0_trace_num+f0_wait_num + 2);

	pr_info("%s:first f0_repeat_num  = %d\n", __func__, f0_repeat_num);
	if (f0_repeat_num <= 0) {
	...
	pr_info("%s:finally f0_repeat_num  = %d\n", __func__, f0_repeat_num);
	aw8624_i2c_write(aw8624,
			AW8624_REG_NUM_F0_1,
	...
	if(aw8624_dts_data.aw8624_f0_count_cur == 0)
		aw8624->f0_data = aw8624->f0;
...
static int aw8624_haptic_f0_calibration(struct aw8624 *aw8624)
{
	...
	if (aw8624_dts_data.aw8624_f0_count > 1) { /*multi cali*/
		pr_info("%s multi f0 cali\n", __func__);
	} else { /*once cali*/
		pr_info("%s once f0 cali\n", __func__);
	...
	if (aw8624_haptic_get_f0(aw8624)) {
		...
		(100-aw8624_dts_data.aw8624_f0_cali_percen)) {
			pr_err("%s Out of calibration range <\n", __func__);
			aw8624->f0_calib_data = 32;
		...
		if (reg_val) {
			...
			if (f0_cali_lra >= 32) {
				temp = reg_val - (64 - f0_cali_lra);
				if(temp <= 31 && temp >= 1 && reg_val >= 32)
					temp = 32;
				if(temp < 0)
					temp = f0_cali_lra + reg_val;
			} else if (f0_cali_lra <= 31 && f0_cali_lra > 0) {
				temp = (reg_val + f0_cali_lra) & 0x3f;
				if(temp >= 32 && reg_val <= 31)
					temp = 31;
			} else {
				temp = reg_val;
			}

			aw8624->f0_calib_data = (int)temp;
		} else {
			aw8624->f0_calib_data = (int)f0_cali_lra;
		}

		pr_info("%s:aw8624_f0_count_cur = %d, get_f0=%d, f0_cali_lra=%d, reg_val = %d, f0_calib_data = %d\n", __func__, aw8624_dts_data.aw8624_f0_count_cur,
		    aw8624->f0,f0_cali_lra, reg_val,aw8624->f0_calib_data);
...
static int aw8624_haptic_init(struct aw8624 *aw8624)
{
	...
	aw8624->mt_midas_duration = 0;
	aw8624->mt_midas_count = 0;
				...
				enum led_brightness value)
{
...
#if 0
	if(!get_switch_status_by_id(SWITCH_MT)) {
		pr_err("%s,missing mt switch!\n",__func__);
		return;
	}
#endif
	mutex_lock(&aw8624->lock);
	...
	aw8624->mt_midas_duration += value;
	aw8624->mt_midas_count += 1;
	if(aw8624->mt_midas_count > INT_MAX || aw8624->mt_midas_duration > LONG_MAX) {
		aw8624->mt_midas_count = 0;
		aw8624->mt_midas_duration = 0;
	}
...
static ssize_t aw8624_mt_midas_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct led_classdev *to_dev = dev_get_drvdata(dev);
	struct aw8624 *aw8624 = container_of(to_dev, struct aw8624, to_dev);

	pr_info("%s: mt_midas = %d, mt_midas_duration = %lld\n", __func__, aw8624->mt_midas_count, aw8624->mt_midas_duration);
	return snprintf(buf, PAGE_SIZE, "%d, %lld\n", aw8624->mt_midas_count, aw8624->mt_midas_duration);
}

static ssize_t aw8624_mt_midas_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct led_classdev *to_dev = dev_get_drvdata(dev);
	struct aw8624 *aw8624 = container_of(to_dev, struct aw8624, to_dev);
...
#if 0
	if(!get_switch_status_by_id(SWITCH_MT)) {
		pr_err("%s,missing mt switch! val:%s\n",__func__, buf);
		return count;
	}
#endif
	rc = kstrtouint(buf, 0, &val);
		...
		char *buf)
{
	...
	//int i = 0;
	int ret = 0;
	aw8624->f0_data = 0;  //clear

	mutex_lock(&aw8624->lock);
	aw8624_haptic_upload_lra(aw8624, AW8624_HAPTIC_ZERO);
	if (aw8624_dts_data.aw8624_f0_count) {
			pr_info("%s f0_count = %d\n", __func__, aw8624_dts_data.aw8624_f0_count);
			for (aw8624_dts_data.aw8624_f0_count_cur = 0; aw8624_dts_data.aw8624_f0_count_cur < aw8624_dts_data.aw8624_f0_count; aw8624_dts_data.aw8624_f0_count_cur++) {
				ret = aw8624_haptic_f0_calibration(aw8624);
				if(ret < 0)
					break;
				usleep_range(200000, 250000);
		}
	} else {
		pr_info("%s dts not config <vib_f0_count> value\n", __func__);
			aw8624_haptic_f0_calibration(aw8624);
	}
	mutex_unlock(&aw8624->lock);

	len += snprintf(buf+len, PAGE_SIZE-len, "%d,%d\n",
			aw8624->f0_data, aw8624->f0_calib_data);
	return len;
}
...
static const DEVICE_ATTR(mt_midas, 0664, aw8624_mt_midas_show, aw8624_mt_midas_store);
...
static irqreturn_t aw8624_irq(int irq, void *data)
{
	...
	/*if (reg_val & AW8624_BIT_SYSINT_FF_AFI)*/
		/*pr_info("%s: aw8624 rtp fifo almost full int\n", __func__);*/
...
static void aw8624_vibrator_work_routine(struct work_struct *work)
{
	...
	if(!get_switch_status_by_id(SWITCH_MT)) {
		pr_err("%s,missing mt switch!\n",__func__);
		/*if wake lock is hold, need release the lock*/
...
static int aw8624_vibrator_init(struct aw8624 *aw8624)
{
	...
	aw8624->to_dev.max_brightness = 1500;
...
static int aw8624_parse_dt(struct device *dev, struct aw8624 *aw8624,
	...
	aw8624->select_gpio = of_get_named_gpio(np, "select-gpio", 0);
	if (aw8624->select_gpio >= 0) {
		dev_info(dev, "%s: reset gpio provided ok\n", __func__);
	} else {
		aw8624->select_gpio = -1;
		dev_err(dev,
		"%s: no reset gpio provided, will not HW reset device\n",
		__func__);
		return -1;
	}
	pr_info("%s reset io(%d), irq io(%d) select io(%d)\n",
			__func__,
			aw8624->reset_gpio,
			aw8624->irq_gpio,
			aw8624->select_gpio);
	...
	val =
	of_property_read_u32_array(np, "vib_sw_brake",
	...
	val =
	of_property_read_u32(np, "vib_f0_count",
				&aw8624_dts_data.aw8624_f0_count);
	if (val != 0)
		pr_info("%s vib_f0_count not found\n", __func__);
...
aw8624_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	/* motor switch enable */
	gpio_direction_output(aw8624->select_gpio, 1);
	...
	pr_info("%s read chip aw8624\n", __func__);

	aw8624->pinctrl = devm_pinctrl_get_select_default(&i2c->dev);
	if (IS_ERR(aw8624->pinctrl)) {
		dev_err(&i2c->dev,
		"%s: pinctrl set failed\n", __func__);
		goto err_id;
	}
	aw8624->pinstate = pinctrl_lookup_state(aw8624->pinctrl, "default");
	if (IS_ERR(aw8624->pinstate)) {
		pr_err("%s: lookup_state failed!\n", __func__);
	}
	...
	aw8624->motor_nb.notifier_call = aw8624_stop_notify_call;
	ret = oplus_switch_notifier_register(SWITCH_MT, aw8624->dev, &aw8624->motor_nb);
...
static void aw8624_force_stop(void)
{
	unsigned char reg_val = 0;
	/* 0x47 glb state
		bit3-bit0 0000 standby, 0111 ram-go, 1000 rtp-go, 1001 trig-go, 0110 cont-go
	*/
	aw8624_i2c_read(g_aw8624, AW8624_REG_GLB_STATE, &reg_val);
	if((reg_val & 0x0f) != 0) {
		pr_info("%s force stop\n", __func__);
		aw8624_haptic_stop(g_aw8624);
	}
}
...
static int aw8624_stop_notify_call(struct notifier_block *nb, unsigned long event, void *data)
{
	...
	//pr_info("%s motor switch callback enable:%d current:%d\n", __func__, pdata[1], pdata[2]);
	if(((event == SWITCH_PRE_ACTION) && (pdata[1] == 0)) 		 /*motor switch to mcu*/
		|| ((event == SWITCH_POST_ACTION) && (pdata[1] != 0))){   /*motor switch to ap*/
		if(get_switch_status_by_id(SWITCH_MT))
			aw8624_force_stop();
		else
			pr_err("%s,missing mt switch!\n",__func__);
