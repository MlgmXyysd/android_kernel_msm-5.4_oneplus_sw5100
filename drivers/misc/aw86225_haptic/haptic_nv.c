			   ...
			   struct device_node *np)
{
	...
	awinic->select_gpio = of_get_named_gpio(np, "select-gpio", 0);
	if (awinic->select_gpio < 0) {
		aw_dev_err("%s: no select gpio provided.\n", __func__);
	} else {
		aw_dev_info("%s: select gpio provided ok.\n", __func__);
	}
...
haptic_nv_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	aw_dev_info("%s aw8624_probe enter\n", __func__);
	...
	/* haptic switch enable */
	gpio_direction_output(awinic->select_gpio, 1);
