...
{
	int ret;
        /*enable user uart start*/
	extern unsigned int is_user_build;
	extern unsigned int user_printk_disable_uart;
	extern unsigned int oplus_ftm_mode;
	if (3 == oplus_ftm_mode) {
	    msm_uart_driver.cons = NULL;
	} else if (user_printk_disable_uart && is_user_build) {
		pr_err("msm_serial_init: driver is needn't initialize\n");
		return 0;
	}
        /*enable user uart end*/
...
static void __exit msm_serial_exit(void)
{
	...
	extern unsigned int user_printk_disable_uart;
	extern unsigned int oplus_ftm_mode;
	if (3 != oplus_ftm_mode && user_printk_disable_uart && is_user_build) {
	    pr_err("msm_serial_init: driver is needn't initialize\n");
	    return ;
	};
        /*enable user uart end*/
