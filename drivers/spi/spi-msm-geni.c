...
struct pm_qos_request spi_pm_qos_req;
				...
				struct spi_transfer *xfer)
{
	...
	if (mas->cur_xfer_mode != GSI_DMA) {
		pm_qos_update_request(&spi_pm_qos_req, 20);
		reinit_completion(&mas->xfer_done);
		...
		if (spi->slave) {
			...
			if (slv->slave_ready_cb) {
				slv->slave_ready_cb(slv, xfer->tx_buf, xfer->rx_buf);
			}
		}
		mutex_unlock(&mas->spi_ssr.ssr_lock);
		timeout = wait_for_completion_timeout(&mas->xfer_done,
	...
	mutex_unlock(&mas->spi_ssr.ssr_lock);
	pm_qos_update_request(&spi_pm_qos_req, PM_QOS_DEFAULT_VALUE);
	return ret;
		...
		if (dma_rx_status & DMA_RX_ERROR_STATUS) {
			...
			if (dma_rx_len != dma_rx_len_in) {
				...
				geni_se_dump_dbg_regs(&mas->spi_rsc, mas->base, mas->ipc);
					...
					GENI_SE_DBG(mas->ipc, false, mas->dev,
					"%s:Data Less, rx_rem:(%d - %d), tx_irq_sts:0x%x rx_irq_sts:0x%x\n",
					__func__, dma_rx_len, dma_rx_len_in, dma_tx_status, dma_rx_status);
					mas->rx_rem_bytes = 0;
				}
			} else {
				mas->rx_rem_bytes = 0;
...
static int spi_geni_resume(struct device *dev)
{
    ...
    if (mem_sleep_current == PM_SUSPEND_MEM){
	   printk("deep sleep spi devices resume\n");
    }
#endif
	return 0;
}
...
{
	pm_qos_add_request(&spi_pm_qos_req, PM_QOS_CPU_DMA_LATENCY, PM_QOS_DEFAULT_VALUE);
