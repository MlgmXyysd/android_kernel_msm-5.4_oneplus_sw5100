...
{
	if (IS_ENABLED(CONFIG_BCMDHD) && host->ops->get_cd && host->ops->get_cd(host))
		return 0;
...
out:
	if (err) {
		/*
		*move mmc_sdio_remove after mmc_claim_host to fix unbanlanced sdcc clk request.
		* first activate the host, the device will resume without clk_req
		*/
