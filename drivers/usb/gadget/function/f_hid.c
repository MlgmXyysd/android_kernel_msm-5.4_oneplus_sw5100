...
static void hidg_unbind(struct usb_configuration *c, struct usb_function *f)
{
	...
	if(hidg->opened)
	{
		pr_err("=====>hid is opened=====");
		if (down_timeout(&hidg->wait_sem,msecs_to_jiffies(2000)) >= 0)
		{
			pr_err("=====>hid is closed=====");
			hidg->opened = false ;
			device_destroy(hidg_class, MKDEV(major, hidg->minor));
			cdev_del(&hidg->cdev);
		}else
		{
			pr_err("=====>hid is close timeout=====");
		}
	}else{
		hidg->opened = false ;
		device_destroy(hidg_class, MKDEV(major, hidg->minor));
