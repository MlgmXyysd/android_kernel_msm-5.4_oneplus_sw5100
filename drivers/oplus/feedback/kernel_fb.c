...
static volatile unsigned int kevent_pid = 0;
...
int oplus_kevent_fb(fb_tag tag_id, const char *event_id, unsigned char *payload)
{
	...
	if (g_pkts->packet_cnt > MAX_PKG) {
		pr_err("packet poll is full.\n");
		return -ENODEV;
	}
	...
	if (kevent_pid == 0) {
		pr_err("waiting for genetlink client register, packet_cnt: %d\n", g_pkts->packet_cnt);
		return 0;
	}
	wake_up_process(g_pkts->flush_task);
