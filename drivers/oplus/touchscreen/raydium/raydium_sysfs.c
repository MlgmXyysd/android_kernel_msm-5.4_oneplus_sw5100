				     ...
				     char *p_i8_buf)
{
	...
	if (i32_ret == 1) {
		snprintf(p_i8_buf, PAGE_SIZE, "Status 1\n");
	} else if (i32_ret == 5) {
		snprintf(p_i8_buf, PAGE_SIZE, "Status 5\ni2c err\n");
	} else if (i32_ret == 6) {
		snprintf(p_i8_buf, PAGE_SIZE, "Status 6\nFinish Cm/Cp test! Range Check Failure\n");
	} else {
		snprintf(p_i8_buf, PAGE_SIZE, "Status 2\nSelf_test_init() fail\n");
	}

	return strlen(p_i8_buf) + 1;
}

static ssize_t raydium_selftest_result_show(struct device *dev,
				     struct device_attribute *attr,
				     char *p_i8_buf)
{
	LOGD(LOG_INFO, "[touch]selftest result\n");
	return self_test_result_read_from_buffer(p_i8_buf, PAGE_SIZE);
}
...
static DEVICE_ATTR(raydium_log_level, 0644,
		   NULL,
		   raydium_log_level_store);/* show the fw version (R)
...
static DEVICE_ATTR(raydium_do_selftest, 0644,
		   ...
		   NULL);
static DEVICE_ATTR(raydium_selftest_result, 0444,
		   raydium_selftest_result_show,
		   NULL);
#endif
...
struct attribute *raydium_attributes[] = {
	...
	&dev_attr_raydium_selftest_result.attr,
#endif
	NULL
};
