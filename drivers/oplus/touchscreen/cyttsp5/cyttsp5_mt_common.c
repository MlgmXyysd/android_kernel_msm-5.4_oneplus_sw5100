		...
		struct cyttsp5_touch *tch, int num_cur_tch)
{
	...
	int sig;
	int i, j, t = 0;
	DECLARE_BITMAP(ids, CY_TOUCH_ID_MAX);
	int mt_sync_count = 0;
	u8 *tch_addr;

	if (CY_TOUCH_ID_MAX < si->tch_abs[CY_TCH_T].max) {
		dev_err(dev,
			"%s: Touch ID num %d is allocated less than needed %d\n",
			__func__, CY_TOUCH_ID_MAX, si->tch_abs[CY_TCH_T].max);
		return;
	}

	bitmap_zero(ids, CY_TOUCH_ID_MAX);
...
static int cyttsp5_xy_worker(struct cyttsp5_mt_data *md)
{
	...
	if (tch.hdr[CY_TCH_LO]) {
		...
		if ((cd->fb_blank !=  MSM_DRM_BLANK_POWERDOWN/*== MSM_DRM_BLANK_UNBLANK*/) && (cd->large_tp_enable)) {
			input_report_key(md->input, KEY_SLEEP, 1);
			input_report_key(md->input, KEY_SLEEP, 0);
			input_sync(md->input);
			dev_info(dev, "Tp large trigger KEY_SLEEP");
		}
#endif
	}
...
int cyttsp5_mt_report_wakeup_key(struct cyttsp5_core_data *cd)
{
	struct cyttsp5_mt_data *md = &cd->md;
	mutex_lock(&md->mt_lock);
	cd->irq_report_wakeup = 1;
	input_report_key(md->input, KEY_F12, 1);
	input_report_key(md->input, KEY_F12, 0);
	input_sync(md->input);
	dev_err(md->dev, "==touch to wakeup KEY_F12 ==\n");
	mutex_unlock(&md->mt_lock);
	return 0;
}
...
static int cyttsp5_mt_wake_attention(struct device *dev)
{
	...
	mutex_lock(&md->mt_lock);
	if (0 == cd->irq_report_wakeup) {
		input_report_key(md->input, KEY_F12, 1);
		input_report_key(md->input, KEY_F12, 0);
		input_sync(md->input);
		dev_err(dev, "==touch to wakeup== KEY_F12\n");
	}
	cd->irq_report_wakeup = 0;
	mutex_unlock(&md->mt_lock);
