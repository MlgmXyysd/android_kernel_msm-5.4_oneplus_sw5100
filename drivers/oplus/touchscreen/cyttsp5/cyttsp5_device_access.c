...
#define CMCP_THRESHOLD_FILE_NAME_VISIONOX_1_74 "ttdl_cmcp_thresholdfile_visionox_1_74.csv"
#define CMCP_THRESHOLD_FILE_NAME_VISIONOX_1_91 "ttdl_cmcp_thresholdfile_visionox_1_91.csv"
#define CMCP_THRESHOLD_FILE_NAME_TM_1_43       "ttdl_cmcp_thresholdfile_tm_1_43.csv"
...
struct cyttsp5_device_access_data {
	...
	struct proc_dir_entry *base_proc_entry;
};
...
struct cyttsp5_device_access_data * proc_cyttsp5_device_access_data = NULL;
...
static const DEVICE_ATTR(status, S_IRUSR, cyttsp5_status_show, NULL);
...
static const DEVICE_ATTR(response, S_IRUSR, cyttsp5_response_show, NULL);
...
static const DEVICE_ATTR(command, S_IWUSR, NULL, cyttsp5_command_store);
	...
	struct cmcp_data *cmcp_info)
{
	...
	if (cp_btn_data != NULL) {
		...
		if (cp_btn_ave != 0) {
			...
			dev_err(dev, "cp_btn_ave == 0");
			cmcp_info->cp_button_delta = 0;
		}
...
static const DEVICE_ATTR(cmcp_test, S_IRUGO | S_IWUSR | S_IWGRP,
	...
	int no_builtin_file)
{
	...
	if ((NULL == configuration) || (NULL == result) || (NULL == cmcp_info)){
		dev_err(dev, "config is %p;result is %p; cmcp_info is %p;",
				configuration, result, cmcp_info);
		return -EINVAL;
	}
...
static ssize_t cyttps5_selftest_results_proc_read(struct file *filp,
		char __user *buf, size_t count, loff_t *ppos)
{
	struct cyttsp5_device_access_data *dad = NULL;
	struct device *dev;
	struct cmcp_data *cmcp_info;
	struct result *result;
	struct configuration *configuration;
	...
	if (proc_cyttsp5_device_access_data == NULL) {
		return 0;
	}

	dad = proc_cyttsp5_device_access_data;
	cmcp_info = dad->cmcp_info;
	result = dad->result;
	configuration = dad->configs;
...
static const struct file_operations cyttsp5_selftest_results_proc_fops = {
	.read = cyttps5_selftest_results_proc_read,
	.write = NULL,
};
...
static const DEVICE_ATTR(cmcp_threshold_loading, S_IRUSR | S_IWUSR,
...
static const BIN_ATTR(cmcp_threshold_data, S_IWUSR, NULL, cyttsp5_cmcp_threshold_data_write, 0);
		...
		bool get_result_on_pass)
{
	...
	if (rc < 0) {
		dev_err(dev, "%s: Error on suspend scan r=%d, retry\n",
				__func__, rc);
		for (retry_time = 0; retry_time < 5; retry_time++) {
			rc = cyttsp5_suspend_scan_cmd_(dev);
			if (rc < 0) {
				dev_err(dev, "%s: Error on suspend scan r=%d, retry %d\n",
						__func__, rc, retry_time);
			} else {
				break;
			}
		}
	...
	if (rc < 0) {
		dev_err(dev, "%s: Error on run self test for test_id:%d r=%d, retry\n",
				__func__, test_id, rc);
		for (retry_time = 0; retry_time < 5; retry_time++) {
			rc = cyttsp5_run_selftest_cmd_(dev, test_id, 0,
					&cmd_status, &summary_result, NULL);
			if (rc < 0) {
				dev_err(dev, "%s: Error on run self test for test_id:%d r=%d, retry %d\n",
						__func__, test_id, rc, retry_time);
			} else {
				break;
			}
		}
	...
	if (rc < 0) {
		dev_err(dev, "%s: Error on get self test result r=%d, retry\n",
				__func__, rc);
		for (retry_time = 0; retry_time < 5; retry_time++) {
			rc = cyttsp5_get_selftest_result_cmd_(dev, 0, read_length,
					test_id, &cmd_status, &act_length, &dad->ic_buf[6]);
			if (rc < 0) {
				dev_err(dev, "%s: Error on get self test result r=%d, retry %d\n",
						__func__, rc, retry_time);
			} else {
				break;
			}
	...
	put_unaligned_le16(act_length, &dad->ic_buf[4]);
		...
		size_t count, loff_t *ppos)
{
	...
	put_unaligned_le16(act_length, &dad->ic_buf[2]);
...
static const struct attribute *cyttsp5_device_access_attributes[] = {
	&dev_attr_command.attr,
	&dev_attr_status.attr,
	&dev_attr_response.attr,
	&dev_attr_cmcp_test.attr,
	&dev_attr_cmcp_threshold_loading.attr,
	NULL
};

static const struct bin_attribute *cyttsp5_device_access_bin_attributes[] = {
	&bin_attr_cmcp_threshold_data,
	NULL
};

static const struct attribute_group cyttsp5_device_access_attribute_group = {
	.attrs = (struct attribute **) cyttsp5_device_access_attributes,
	.bin_attrs = (struct bin_attribute **) cyttsp5_device_access_bin_attributes,
};
...
static const struct {
	...
	{ "panel_scan",   0600, &panel_scan_debugfs_fops, },
	{ "get_idac",     0600, &get_idac_debugfs_fops, },
	{ "auto_shorts",  0400, &auto_shorts_debugfs_fops, },
	{ "opens",        0400, &opens_debugfs_fops, },
	{ "calibrate",    0600, &calibrate_debugfs_fops, },
	{ "baseline",     0600, &baseline_debugfs_fops, },
	{ "cm_panel",     0400, &cm_panel_debugfs_fops, },
	{ "cp_panel",     0400, &cp_panel_debugfs_fops, },
	{ "cm_button",    0400, &cm_button_debugfs_fops, },
	{ "cp_button",    0400, &cp_button_debugfs_fops, },
	{ "cmcp_results", 0644, &cmcp_results_debugfs_fops, },
};
...
static int cyttsp5_setup_sysfs(struct device *dev)
{
	...
	rc = sysfs_create_group(&dev->kobj, &cyttsp5_device_access_attribute_group);
	if (rc) {
		dev_err(dev, "%s: Error, could not create sysfs group\n",
				__func__);
		goto exit;
	}
	...
	for (i = 0; i < ARRAY_SIZE(mfg_test_files); i++) {
		if (IS_ERR_OR_NULL(debugfs_create_file(mfg_test_files[i].name, mfg_test_files[i].mode,
			dad->mfg_test_dentry, dad, mfg_test_files[i].fops))) {
			dev_err(dev, "%s: Error, could not create %s\n",
					__func__, mfg_test_files[i].name);
			goto unregister_base_dir;
		}
	}
...
#endif
register_proc_group:
	dad->base_proc_entry = proc_mkdir("oplusTouchscreen", NULL);
	if (IS_ERR_OR_NULL(dad->base_proc_entry)) {
		dev_err(dev, "%s: Error, could not create base proc directory\n",
				__func__);
		goto unregister_base_dir;
	}

	proc_cyttsp5_device_access_data = dad;

	if (IS_ERR_OR_NULL(proc_create("selftest_result", S_IRUGO,
		dad->base_proc_entry, &cyttsp5_selftest_results_proc_fops))) {
		dev_err(dev, "%s: Error, could not create proc %s\n",
				__func__, "selftest_result");
		goto unregister_base_proc_dir;
	}

	dad->sysfs_nodes_created = true;
	return rc;

unregister_base_proc_dir:
	proc_cyttsp5_device_access_data = NULL;
	remove_proc_entry("oplusTouchscreen", NULL);
unregister_base_dir:
	if (!IS_ERR_OR_NULL(dad->base_dentry)) {
		debugfs_remove_recursive(dad->base_dentry);
	}
	sysfs_remove_group(&dev->kobj, &cyttsp5_device_access_attribute_group);
	dev_err(dev, "%s: Error, remove sysfs group!\n", __func__);
exit:
	return rc;
}
	...
	struct work_struct *cmcp_threshold_update)
{
	...
	/* Open threshold file */
	switch(oplus_panel_id) {
		case 0x098001:
		case 0x098002:
			retval = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
				CMCP_THRESHOLD_FILE_NAME_VISIONOX_1_91, dev, GFP_KERNEL, dev,
		...
		case 0x097006:
			retval = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
				CMCP_THRESHOLD_FILE_NAME_VISIONOX_1_74, dev, GFP_KERNEL, dev,
				cyttsp5_cmcp_parse_threshold_file);
			break;
		case 0x544D32:
			retval = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
				CMCP_THRESHOLD_FILE_NAME_TM_1_43, dev, GFP_KERNEL, dev,
				cyttsp5_cmcp_parse_threshold_file);
			break;
		default:
			retval = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
