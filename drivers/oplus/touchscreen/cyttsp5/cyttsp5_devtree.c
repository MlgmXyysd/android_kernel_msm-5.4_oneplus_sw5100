...
#define VISIONOX_402X476_CMD_SIGNATURE_1 0x098001
#define VISIONOX_402X476_CMD_SIGNATURE_2 0x098002
...
fail_del_kobj:
	...
	name = NULL; //sysfs_create_file() delete vkeys->kobj_attr.attr.name = name
fail_free_name:
	if (name != NULL) {
		kfree(name);
	}
...
static void *create_and_get_mt_pdata(struct device_node *dev_node)
{
	...
	if (!rc) {
		pdata->flags = value;
		if (oplus_panel_id == 0x048005)
		{
			pdata->flags |= 0x10;
		}
		else if (oplus_panel_id == EVERDISPLAY_320X360_CMD_SIGNATURE)
		{
			pdata->flags |= 0x00;
		}
		else if ((oplus_panel_id == VISIONOX_402X476_CMD_SIGNATURE_1)
			|| (oplus_panel_id == VISIONOX_402X476_CMD_SIGNATURE_2))
		{
			pdata->flags |= 0x08;
		}
		pr_err("%s: panel_id=%x,flag=%x\n", __func__,oplus_panel_id,pdata->flags);
	}
...
static int cyttsp5_setup_pinctrl(struct device *dev, struct cyttsp5_core_platform_data *pdata)
{
	/* Get pinctrl if target uses pinctrl */
	pdata->pinctrl = devm_pinctrl_get(dev);
	if (IS_ERR(pdata->pinctrl)) {
		if (PTR_ERR(pdata->pinctrl) == -EPROBE_DEFER)
			return -EPROBE_DEFER;

		pr_debug("Target does not use pinctrl\n");
		pdata->pinctrl = NULL;
	} else {
		int rc = cyttsp5_pinctrl_configure(dev, pdata->pinctrl, "pmx_ts_active");
		if (rc < 0)
			return rc;
	}
...
static int cyttsp5_setup_regulator(struct device *dev, struct cyttsp5_core_platform_data *pdata)
{
	int rc;

	pdata->vdd_reg = devm_regulator_get(dev, "vdd");
	if (IS_ERR(pdata->vdd_reg)) {
		if (PTR_ERR(pdata->vdd_reg) == -EPROBE_DEFER)
			return -EPROBE_DEFER;

		pr_debug("%s could not get VDD", __func__);
		pdata->vdd_reg = NULL;
	}

	pdata->vddio_reg = devm_regulator_get(dev, "vddio");
	if (IS_ERR(pdata->vddio_reg)) {
		if (PTR_ERR(pdata->vddio_reg) == -EPROBE_DEFER)
			return -EPROBE_DEFER;

		pr_debug("%s could not get VDD-IO", __func__);
		pdata->vddio_reg = NULL;
	}

	pdata->bus_pullup_reg = devm_regulator_get_optional(dev, "bus-pull");
	if (IS_ERR(pdata->bus_pullup_reg)) {
		if (PTR_ERR(pdata->bus_pullup_reg) == -EPROBE_DEFER)
			return -EPROBE_DEFER;

		pr_debug("%s could not get VDD-BUS-PULL-UP", __func__);
		pdata->bus_pullup_reg = NULL;
	}
...
_err2:
	if (pdata->vddio_reg != NULL)
		regulator_disable(pdata->vddio_reg);
_err1:
	if (pdata->vdd_reg != NULL)
...
static struct cyttsp5_core_platform_data *create_and_get_core_pdata(
		struct device *adap_dev, struct device_node *core_node)
{
	...
	rc = cyttsp5_setup_regulator(adap_dev, pdata);
	if (rc < 0)
		goto fail_regulator;

	/* Required fields */
	value = of_get_named_gpio_flags(core_node, "cy,irq_gpio", 0, &gpio_flag);
	 ...
	 */
	value = of_get_named_gpio_flags(core_node, "cy,rst_gpio", 0, &gpio_flag);
	...
	rc = cyttsp5_setup_pinctrl(adap_dev, pdata);
	if (rc < 0)
		goto fail_pinctrl;
...
fail_regulator:
	if (pdata->pinctrl != NULL)
		cyttsp5_pinctrl_configure(adap_dev, pdata->pinctrl, "pmx_ts_release");
fail_pinctrl:
fail_free:
	kfree(pdata);
fail:
...
static void free_core_pdata(struct device *adap_dev, struct cyttsp5_core_platform_data *pdata)
{
	unsigned int i;
	if (NULL == pdata) {
...
int cyttsp5_devtree_create_and_get_pdata(struct device *adap_dev)
{
	...
	for_each_child_of_node(adap_dev->of_node, core_node) {
		...
		if (rc) {
			free_core_pdata(adap_dev, pdata->core_pdata);
