				 ...
				 struct cyttsp5_hex_image *row_image)
{
	...
	if (length > CY_DATA_MAX_ROW_SIZE) {
		dev_err(dev, "%s: length %d is larger than CY_DATA_MAX_ROW_SIZE(%d)\n",
			__func__, length ,CY_DATA_MAX_ROW_SIZE);
		return -EINVAL;
	}

	data = kzalloc(length, GFP_KERNEL);
	if (!data) {
		dev_err(dev, "%s: kzalloc data failed\n", __func__);
		return -ENOMEM;
	}
	data[0] = row_image->array_id;
	put_unaligned_le16(row_image->row_num, &data[1]);
		...
		int fw_size)
{
	...
	int rc;
	bool create_exclusive_success = false;
	...
	ld->loader_pdata->flags |= CY_LOADER_FLAG_CALIBRATE_AFTER_FW_UPGRADE; //OPLUS_DRIVER_UPDATE_1122, Always do calibration after firmware upgrade.
...
exit:
	if (!rc)
		oplus_cyttsp5_force_restart_directly(dev, true);
	if (create_exclusive_success) {
		cmd->release_exclusive(dev);
	}
...
static int cyttsp5_check_ttconfig_version_builtin(struct device *dev,
		const struct firmware *fw)
{
	struct cyttsp5_loader_data *ld = cyttsp5_get_loader_data(dev);
	u16 conf_ver_new;
	...
	conf_ver_new = get_unaligned_be16(fw->data + 17);
	conf_ver_img = ld->si->cydata.fw_ver_conf;

	parade_debug(dev, DEBUG_LEVEL_1,
		"%s: img vers:0x%04X new vers:0x%04X\n", __func__,
			conf_ver_img, conf_ver_new);

	if (conf_ver_new > conf_ver_img) {
		parade_debug(dev, DEBUG_LEVEL_1,
			"%s: Image is newer, will upgrade\n", __func__);
		return 1;
	}
		...
		void *context)
{
	...
	if (!upgrade)
		upgrade = cyttsp5_check_ttconfig_version_builtin(dev, fw);
	if (upgrade) {
...
#define FILENAME_LEN_MAX 64
	...
	if (panel_id == PANEL_ID_NOT_ENABLED) {
		switch(oplus_panel_id) {
			case 0x098001:
			case 0x098002:
				snprintf(filename, FILENAME_LEN_MAX, "%s", CY_FW_FILE_NAME_VISIONOX_1_91);
				break;
			case 0x097004:
			case 0x097005:
			case 0x097006:
				snprintf(filename, FILENAME_LEN_MAX, "%s", CY_FW_FILE_NAME_VISIONOX_1_74);
				break;
			case 0x544D32:
				snprintf(filename, FILENAME_LEN_MAX, "%s", CY_FW_FILE_NAME_TM_1_43);
				break;
			default:
...
static const BIN_ATTR(config_data, S_IWUSR, NULL, cyttsp5_config_data_write, 0);
...
static const DEVICE_ATTR(config_loading, S_IRUSR | S_IWUSR,
		...
		struct work_struct *fw_and_config_upgrade)
{
	...
	if (MSM_BOOT_MODE__FACTORY == boot_mode) {
		dev_info(dev, "%s: Do not upgrade TP firmware in ftm mode!\n", __func__);
		return;
	}
...
static const DEVICE_ATTR(forced_upgrade, S_IWUSR,
...
static const DEVICE_ATTR(manual_upgrade, S_IWUSR,
	NULL, cyttsp5_manual_upgrade_store);
#endif

static const struct attribute *cyttsp5_loader_attributes[] = {
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_PLATFORM_FW_UPGRADE
	&dev_attr_forced_upgrade.attr,
#endif
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_BINARY_FW_UPGRADE
	&dev_attr_manual_upgrade.attr,
#endif
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_MANUAL_TTCONFIG_UPGRADE
	&dev_attr_config_loading.attr,
#endif
	NULL
};

static const struct bin_attribute *cyttsp5_loader_bin_attributes[] = {
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_MANUAL_TTCONFIG_UPGRADE
	&bin_attr_config_data,
#endif
	NULL
};

static const struct attribute_group cyttsp5_loader_attribute_group = {
	.attrs = (struct attribute **) cyttsp5_loader_attributes,
	.bin_attrs = (struct bin_attribute **) cyttsp5_loader_bin_attributes,
};
...
static int cyttsp5_loader_probe(struct device *dev, void **data)
{
	...
	rc = sysfs_create_group(&dev->kobj, &cyttsp5_loader_attribute_group);
	if (rc) {
		dev_err(dev, "%s: Error, could not create sysfs group\n",
				__func__);
		goto error_create_sysfs_group;
	}
...
static void cyttsp5_loader_release(struct device *dev, void *data)
{
	...
	kfree(ld->config_data);
#endif
	sysfs_remove_group(&dev->kobj, &cyttsp5_loader_attribute_group);
	kfree(ld);
}
