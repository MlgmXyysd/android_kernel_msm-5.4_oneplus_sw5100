...
MODULE_FIRMWARE(CY_FW_FILE_NAME_VISIONOX_1_74);
MODULE_FIRMWARE(CY_FW_FILE_NAME_VISIONOX_1_91);
...
//add resume report key
extern int cyttsp5_mt_report_wakeup_key(struct cyttsp5_core_data *cd);
		...
		const u8 *data_name)
{
	...
	if (cd->show_timestamp) {
		memset(data_name_with_time_stamp, 0, sizeof(data_name_with_time_stamp));
		snprintf(data_name_with_time_stamp, sizeof(data_name_with_time_stamp) - 1, "[%u] %s",
...
static inline u16 _cyttsp5_compute_crc(u8 *buf, u32 size)
{
	return crc_itu_t(0xFFFF, buf, size);
}
		...
		struct cyttsp5_hid_output *hid_output)
{
	...
	if (get_unaligned_le16(&cd->response_buf[size - 3]) != crc) {
		...
		struct cyttsp5_hid_output *hid_output)
{
	...
	if (command_code != hid_output->command_code) {
		dev_err(cd->dev, "%s: HID output response, wrong command_code:%X\n",
		...
		struct cyttsp5_hid_output *hid_output, bool raw)
{
	...
	if (raw && hid_output->length >= 10 && hid_output->length <= 13
			...
			&& hid_output->write_buf[4] == HID_APP_OUTPUT_REPORT_ID
	...
	if (cd->response_buf[2] != HID_APP_RESPONSE_REPORT_ID
			|| (cd->response_buf[4] & HID_OUTPUT_CMD_MASK) != HID_OUTPUT_SET_PARAM
		...
		struct cyttsp5_hid_output *hid_output)
{
	...
	put_unaligned_le16(length, &cmd[cmd_offset]);
	cmd_offset += 2;
	...
	if (hid_output->cmd_type == HID_OUTPUT_CMD_BL) {
		put_unaligned_le16(hid_output->write_length, &cmd[cmd_offset]);
		cmd_offset += 2;
	}
	/* Set Data */
	...
	if (hid_output->cmd_type == HID_OUTPUT_CMD_BL) {
		...
		put_unaligned_le16(crc, &cmd[cmd_offset]);
		cmd_offset += 2;
...
static int cyttsp5_si_setup(struct cyttsp5_core_data *cd)
{
	...
	ptr = krealloc(si->xy_data, max_tch * si->desc.tch_record_size, GFP_KERNEL);
	if (!ptr)
		goto _out;
	si->xy_data = ptr;

	ptr = krealloc(si->xy_mode, si->desc.tch_header_size, GFP_KERNEL);
	if (!ptr)
		goto _out;
	si->xy_mode = ptr;

	memset(si->xy_data, 0, max_tch * si->desc.tch_record_size);
	memset(si->xy_mode, 0, si->desc.tch_header_size);
...
static int cyttsp5_si_get_btn_data(struct cyttsp5_core_data *cd)
{
	...
	int btn;
	int rc = 0;
	u8 btns = cd->response_buf[HID_SYSINFO_BTN_OFFSET] & HID_SYSINFO_BTN_MASK;
	int num_btns = hweight8(btns);
	...
	if (num_btns) {
		...
		if (cd->cpdata->sett[CY_IC_GRPNUM_BTN_KEYS] == NULL)
			...
			num_defined_keys = cd->cpdata->sett[CY_IC_GRPNUM_BTN_KEYS]->size;

		for (btn = 0; btn < num_btns && btn < num_defined_keys; btn++) {
			key_table = (u16 *)cd->cpdata->sett[CY_IC_GRPNUM_BTN_KEYS]->data;
		...
		struct cyttsp5_core_data *cd, u8 data, u8 *return_data)
{
	int rc;
	struct cyttsp5_hid_output hid_output = {
		HID_OUTPUT_APP_COMMAND(HID_OUTPUT_ALWAYS_ACTIVE_MODE),
		.write_length = 1,
		.write_buf = &data,
	};
		...
		u16 row_number, u16 length, u8 ebid, u8 *read_buf, u16 *crc)
{
	...
	put_unaligned_le16(row_number, &write_buf[0]);
		...
		u8 *security_key, u16 *actual_write_len)
{
	...
	const int full_write_length = 2 + 2 + 1 + write_length + 8 + 2;
	u8 *full_write_buf;
	u8 cmd_offset = 0;
	u16 crc;
	int status;
	...
	put_unaligned_le16(row_number, &full_write_buf[0]);
	put_unaligned_le16(write_length, &full_write_buf[2]);
	full_write_buf[4] = ebid;
	cmd_offset = 5;
	...
	put_unaligned_le16(crc, &full_write_buf[cmd_offset]);
...
again:
	put_unaligned_le16(read_offset, &write_buf[0]);
	put_unaligned_le16(read_length, &write_buf[2]);
	write_buf[4] = data_id;
		...
		u8 *read_buf)
{
	...
	put_unaligned_le16(read_offset, &write_buf[0]);
	put_unaligned_le16(read_count, &write_buf[2]);
	write_buf[4] = data_id;
...
static u16 oplus_cyttsp5_get_config_ver_(struct cyttsp5_core_data *cd)
{
	int rc;
	u16 config_ver = 0xF5FA;

	rc = cyttsp5_hid_output_suspend_scanning_(cd);
	if (rc) {
		dev_err(cd->dev, "%s: cyttsp5_hid_output_suspend_scanning_() fail return %d\n", __func__, rc);
		goto error;
	}

	rc = cyttsp5_hid_output_read_conf_ver_(cd, &config_ver);
	if (rc) {
		dev_err(cd->dev, "%s: cyttsp5_hid_output_read_conf_ver_() fail return %d\n", __func__, rc);
		goto exit;
	}
		...
		struct cyttsp5_core_data *cd, u8 *panel_id)
{
	...
	if (rc == -EPROTO && cd->response_buf[5] == ERROR_COMMAND) {
		parade_debug(cd->dev, DEBUG_LEVEL_1, "%s: Get Panel ID command not supported\n",
			__func__);
...
{
	if (cd->cpdata->xres) {
		int rc = cd->cpdata->xres(cd->cpdata, cd->dev);
	...
	while (buf < end) {
		...
		case HID_ITEM_TYPE_GLOBAL:
			...
			case HID_GLOBAL_ITEM_TAG_USAGE_PAGE:
				...
				usage_page = (u16)get_hid_item_data(data, item_size);
				break;
			case HID_GLOBAL_ITEM_TAG_LOGICAL_MINIMUM:
				...
				logical_min = get_hid_item_data(data, item_size);
				break;
			case HID_GLOBAL_ITEM_TAG_LOGICAL_MAXIMUM:
				...
				logical_max = get_hid_item_data(data, item_size);
				break;
			case HID_GLOBAL_ITEM_TAG_REPORT_COUNT:
				...
				report_count = get_hid_item_data(data, item_size);
				break;
			case HID_GLOBAL_ITEM_TAG_REPORT_SIZE:
				...
				report_size = get_hid_item_data(data, item_size);
				break;
			default:
				dev_info(cd->dev, "%s: Unrecognized Global tag %d\n",
					__func__, item_tag);
			}
			break;
		case HID_ITEM_TYPE_LOCAL:
			...
			case HID_LOCAL_ITEM_TAG_USAGE:
				...
				usage = (u16)get_hid_item_data(data, item_size);
				break;
			...
			case HID_LOCAL_ITEM_TAG_USAGE_MAXIMUM:
				...
				break;
			default:
				dev_info(cd->dev, "%s: Unrecognized Local tag %d\n",
					__func__, item_tag);
			}
			break;
		case HID_ITEM_TYPE_MAIN:
			...
			case HID_MAIN_ITEM_TAG_BEGIN_COLLECTION:
				...
				if (CY_HID_MAX_NESTED_COLLECTIONS == collection_nest) {
					rc = -EINVAL;
					goto exit;
				}
				...
				collection_types[collection_nest] = get_hid_item_data(data, item_size);

				if (collection_types[collection_nest] == HID_COLLECTION_LOGICAL)
...
continue_main_item:
				...
				report = cyttsp5_get_hid_report_(cd, report_type, report_id, true);
				if (!report) {
					rc = -ENOMEM;
					goto exit;
				}
				if (!report->usage_page && collection_nest > 0)
					report->usage_page = collection_usages[collection_nest - 1];
				...
				field->offset = offset;
				field->data_type = get_hid_item_data(data, item_size);
				field->logical_min = logical_min;
				field->logical_max = logical_max;
				field->usage_page = up_usage;

				for (i = 0; i < collection_nest; i++) {
					field->collection_usage_pages[collection_types[i]] = collection_usages[i];
				}
				...
				if (logical_collection_count == 1) {
					...
					if (report->record_field_index == 0)
						report->record_field_index = report->num_fields - 1;
		...
		u32 collection_usage_page)
{
	...
	for (i = 0; i < cd->num_hid_reports; i++) {
		...
		for (j = 0; j < report->num_fields; j++) {
			...
			field_cup = field->collection_usage_pages[HID_COLLECTION_LOGICAL];
		...
		struct cyttsp5_hid_field *field)
{
	...
	tch_abs->size = (field->report_size + 7) / 8;
	tch_abs->min = 0;
	...
	for (t = 0; t < cd->num_hid_reports; t++) {
		...
		for (j = 0; j < report->num_fields; j++) {
			...
			parade_debug(cd->dev, DEBUG_LEVEL_2, "  Collections Phys:%08X App:%08X Log:%08X\n",
				field->collection_usage_pages[HID_COLLECTION_PHYSICAL],
				field->collection_usage_pages[HID_COLLECTION_APPLICATION],
				field->collection_usage_pages[HID_COLLECTION_LOGICAL]);
		}
	}
...
static int cyttsp5_put_device_into_deep_sleep_(struct cyttsp5_core_data *cd)
{
	...
	if (cd->cpdata->pinctrl != NULL) {
		rc = cyttsp5_pinctrl_configure(cd->dev, cd->cpdata->pinctrl, "pmx_ts_suspend");
		if (rc < 0)
			return rc;
	}
...
static int cyttsp5_put_device_into_sleep_(struct cyttsp5_core_data *cd)
{
	...
	flag = IS_DEEP_SLEEP_CONFIGURED(cd->easy_wakeup_gesture);
	flag |= !cd->easy_wakeup_enable;
	if (flag)
		rc = cyttsp5_put_device_into_deep_sleep_(cd);
	else
		rc = cyttsp5_put_device_into_easy_wakeup_(cd);
	if (rc < 0)
		dev_err(cd->dev, "%s: %s rc = %d\n",
				__func__, flag ? "cyttsp5_put_device_into_deep_sleep_" : "cyttsp5_put_device_into_easy_wakeup_", rc);
	return rc;
}
...
static int cyttsp5_core_sleep_(struct cyttsp5_core_data *cd)
{
	...
	if (SS_ALWAYS_ACTIVE_DIS != cd->always_active_mode) {
		...
		mutex_unlock(&cd->system_lock);
		rc = cyttsp5_hid_output_set_always_active_mode_(cd,
			cd->always_active_mode, &return_data);
		if(rc) {
			dev_err(cd->dev, "%s: call set_always_active_mode r=%d\n",
				__func__, rc);
		}
	}
...
static int cyttsp5_wakeup_host(struct cyttsp5_core_data *cd)
{
	...
	/* attention WAKE */
	if (event_id != GESTURE_TOUCH_DETECTED && event_id != 0xAA)
		goto exit;
...
exit:
	...
	report_length = get_unaligned_le16(cd->input_buf);
	...
	memcpy(cd->gesture_data, &cd->input_buf[4], cd->gesture_data_length);
...
static int cyttsp5_read_input(struct cyttsp5_core_data *cd)
{
	...
	if (!IS_DEEP_SLEEP_CONFIGURED(cd->easy_wakeup_gesture)) {
		if (cd->sleep_state == SS_SLEEP_ON) {
			...
			if (IS_TMO(t)){
				dev_err(dev, "%s: Error wait i2c control timeout, fake wakeup event.\n",
						__func__);
				cd->input_buf[0] = 0x07;
				cd->input_buf[1] = 0x00;
				cd->input_buf[2] = 0x04;
	...
	int mode)
{
	...
	list_for_each_entry_safe(atten, atten_n, &cd->atten_list[type], node) {
		if (atten->id == id && atten->mode == mode) {
			...
			spin_unlock(&cd->spinlock);
			parade_debug(cd->dev, DEBUG_LEVEL_2, "%s: %s=%p %s=%d\n",
				__func__, "unsub for atten->dev", atten->dev,
		...
		struct cyttsp5_core_data *cd)
{
	...
	if (cd->cpdata->pinctrl != NULL) {
		rc = cyttsp5_pinctrl_configure(cd->dev, cd->cpdata->pinctrl, "pmx_ts_active");
		if (rc < 0)
			return rc;
	}
...
static int cyttsp5_core_wake_device_(struct cyttsp5_core_data *cd)
{
	if (!IS_DEEP_SLEEP_CONFIGURED(cd->easy_wakeup_gesture)) {
		...
		rc = cyttsp5_hid_output_exit_easywake_state_(cd,
					cd->easy_wakeup_gesture, &status);
		parade_debug(cd->dev, DEBUG_LEVEL_1, "%s: rc = %d, status = %d\n",
				__func__, rc, status);
		if (rc != -EPROTO)
			return rc ? rc : (status ? 0 : -EBUSY);
...
reset:
	...
	cd->hid_core.hid_report_desc_len = le16_to_cpu(cd->hid_desc.report_desc_len);
	cd->hid_core.hid_max_input_len = le16_to_cpu(cd->hid_desc.max_input_len);
	cd->hid_core.hid_max_output_len = le16_to_cpu(cd->hid_desc.max_output_len);
...
static int cyttsp5_core_oplus_resume(struct device *dev)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);

	if ((SS_SLEEP_ON == cd->sleep_state) &&(cd->irq == pm_wakeup_irq())) {
	    cyttsp5_mt_report_wakeup_key(cd);
	}
	return 0;
}

const struct dev_pm_ops cyttsp5_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(NULL, cyttsp5_core_oplus_resume)
};
...
static int oplus_cyttsp5_force_power_off_(struct cyttsp5_core_data *cd)
{
	int rc = 0;
	cd->easy_wakeup_enable = 0;
	cd->fb_blank = MSM_DRM_BLANK_POWERDOWN;

	cancel_work_sync(&cd->pwr_work);
	dev_info(cd->dev, "%s: force POWERDOWN!\n", __func__);
...
/* Read fw verion to check connection with TP IC */
static ssize_t oplus_cyttsp5_check_connection_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	ssize_t ret;
	u16 config_ver = oplus_cyttsp5_get_config_ver_(cd);

	if (config_ver >= 0x01 && config_ver <= 0x50) {
		parade_debug(dev, DEBUG_LEVEL_1, "%s: connected, config_ver = %d\n", config_ver);
		ret = snprintf(buf, CY_MAX_PRBUF_SIZE, "0\n");
	} else {
		dev_err(cd->dev, "%s: disconnected, config_ver = %d\n", __func__, config_ver);
		ret = snprintf(buf, CY_MAX_PRBUF_SIZE, "1\n");
	}
 ...
 * power off via sysfs
 */
static ssize_t oplus_cyttsp5_force_power_off_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	int rc;
	dev_err(dev, "%s: enter\n", __func__);
	rc = oplus_cyttsp5_force_power_off_(cd);
	if (rc < 0)
		dev_err(dev, "%s: power off failed r=%d\n",
			__func__, rc);

	return size;
}

static int oplus_cyttsp5_startup(struct cyttsp5_core_data *cd, bool reset)
{
	int rc;
	mutex_lock(&cd->system_lock);
	cd->startup_state = STARTUP_RUNNING;
...
ssize_t oplus_cyttsp5_force_restart_directly(struct device *dev, bool reset)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	int rc;
	parade_debug(dev, DEBUG_LEVEL_2, "%s: start", __func__);
	rc = oplus_cyttsp5_startup(cd, reset);
	if (rc < 0)
		dev_err(dev, "%s: force restart failed r=%d\n", __func__, rc);
	parade_debug(dev, DEBUG_LEVEL_2, "%s: end", __func__);
	return rc;
}
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	length = cyttsp5_ic_parse_input_dec(dev, buf, size, input_data, 3);
	if (length <= 0) {
...
static ssize_t cyttsp5_debug_level_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	unsigned long value;
	...
	dev_info(dev, "%s: value %lu\n", __func__, value);

	if (value >= DEBUG_LEVEL_NUM)
		value = DEBUG_LEVEL_2;

	mutex_lock(&cd->system_lock);
	cd->debug_level = value;
	mutex_unlock(&cd->system_lock);

	return size;
}

static ssize_t cyttsp5_debug_level_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	ssize_t ret;
...
static ssize_t cyttsp5_easy_wakeup__enable_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	unsigned long value;
...
static ssize_t cyttsp5_easy_wakeup__enable_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	ssize_t ret;
	...
	mutex_lock(&cd->system_lock);
	ret = snprintf(buf, CY_MAX_PRBUF_SIZE, "%d\n", cd->easy_wakeup_enable);
	mutex_unlock(&cd->system_lock);

	return ret;
}

static ssize_t cyttsp5_large_tp_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	unsigned long value;
	...
	mutex_lock(&cd->system_lock);
	cd->large_tp_enable = !!value;
	mutex_unlock(&cd->system_lock);

	dev_info(dev, "%s: large_tp %d\n", __func__, cd->large_tp_enable);
	return size;
}


static ssize_t cyttsp5_large_tp_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	ssize_t ret;
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
		...
		struct device_attribute *attr, char *buf)
{
	...
	mutex_lock(&cd->system_lock);
	ret = snprintf(buf, CY_MAX_PRBUF_SIZE, "%d\n", cd->always_active_mode);
	mutex_unlock(&cd->system_lock);

	return ret;
}

static ssize_t cyttsp5_always_active_mode_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t size)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	unsigned long value;
	...
	mutex_lock(&cd->system_lock);
	cd->always_active_mode = !(!value);
	mutex_unlock(&cd->system_lock);
	ret = cyttsp5_hid_output_set_always_active_mode_(cd, cd->always_active_mode, &return_data);

	dev_info(dev, "%s: always_active_mode %d, ret %d, rtd %d\n",
		__func__, cd->always_active_mode, ret, return_data);
	if (ret)
		return -1;
	return size;
}

static const struct device_attribute attributes[] = {
	...
	__ATTR(debug_level, S_IRUGO | S_IWUSR | S_IWGRP,
		cyttsp5_debug_level_show,
		cyttsp5_debug_level_store),
	__ATTR(easy_wakeup_enable, S_IRUGO | S_IWUSR | S_IWGRP,
		cyttsp5_easy_wakeup__enable_show,
		cyttsp5_easy_wakeup__enable_store),
	__ATTR(large_tp_enable, S_IRUGO | S_IWUSR | S_IWGRP,
		cyttsp5_large_tp_show,
		cyttsp5_large_tp_store),

	__ATTR(check_connection, S_IRUGO, oplus_cyttsp5_check_connection_show, NULL),
	__ATTR(force_power_off, S_IWUSR | S_IWGRP, NULL, oplus_cyttsp5_force_power_off_store),
	__ATTR(always_active_mode, S_IRUGO | S_IWUSR | S_IWGRP,
		cyttsp5_always_active_mode_show,
		cyttsp5_always_active_mode_store),
};
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	rc = cyttsp5_probe(cyttsp5_bus_ops_save, &client->dev, client->irq, 512);
	if (!rc) {
...
static const DEVICE_ATTR(ttdl_restart, S_IWUSR, NULL, cyttsp5_ttdl_restart_store);
...
{
	struct cyttsp5_core_data *res = NULL;
...
void cyttsp5_power_resume_work(struct work_struct *work)
{
	int rc = 0;
	struct cyttsp5_core_data *cd = container_of(
			work, struct cyttsp5_core_data,
			pwr_work);
		...
		unsigned long event, void *data)
{
	...
	blank = evdata->data;
	if(event == MSM_DRM_EVENT_IDLE) {
		if(*blank == MSM_DRM_ENTER_IDLE) {
			equivalent_blank = MSM_DRM_BLANK_POWERDOWN;
			blank = &equivalent_blank;
			dev_info(cd->dev, "%s: Handle enter idle event as POWERDOWN!\n", __func__);
		} else if (*blank == MSM_DRM_EXIT_IDLE) {
			equivalent_blank = MSM_DRM_BLANK_UNBLANK;
			blank = &equivalent_blank;
			dev_info(cd->dev, "%s: Handle exit idle as UNBLANK!\n", __func__);
		} else {
			dev_err(cd->dev, "%s: Error:unknown event(%d) and blank(%d)\n",
	...
	if (*blank == MSM_DRM_BLANK_UNBLANK) {
		...
		if (cd->fb_state != FB_ON) {
			...
			cd->fb_state = FB_ON;
#endif
			queue_work(cd->pwr_on_workqueue, &cd->pwr_work);
		}
	} else if (*blank != MSM_DRM_BLANK_UNBLANK /*== MSM_DRM_BLANK_POWERDOWN*/) {
		cancel_work_sync(&cd->pwr_work);
...
static void cyttsp5_setup_fb_notifier(struct cyttsp5_core_data *cd)
{
	...
	rc = msm_drm_register_client(&cd->fb_notifier);
	if (rc)
		...
		u16 irq, size_t xfer_buf_size)
{
...
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_WATCHDOG_TIMER
	cd->watchdog_interval = CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_WATCHDOG_TIMER_INTERVAL;
#endif
	...
	cd->pwr_on_workqueue = create_singlethread_workqueue("tp_pwr_on_wq");
	if(!cd->pwr_on_workqueue)
		dev_err(cd->dev, "%s: create tp power workqueue failed\n", __func__);
...
#endif
	/*Initialize work for tp power on when display on*/
	INIT_WORK(&cd->pwr_work, cyttsp5_power_resume_work);
...
error_detect:
	if (cd->pwr_on_workqueue)
		destroy_workqueue(cd->pwr_on_workqueue);
	if (cd->cpdata->init)
...
#elif defined(CONFIG_FB)
	msm_drm_unregister_client(&cd->fb_notifier);
#endif
