...
char * fts_self_test_result_buffer = NULL;
ssize_t fts_self_test_result_buffer_index = 0;
...
static int fts_test_main_exit(void)
{
    ...
    if (1) {
        if (tdata->func) {
            if (tdata->func->save_data_private) {
    ...
    if (fts_self_test_result_buffer == NULL) {
        fts_self_test_result_buffer = vmalloc(TXT_BUFFER_LEN);
    }
    if (fts_self_test_result_buffer != NULL) {
        memset(fts_self_test_result_buffer, 0, TXT_BUFFER_LEN);
        fts_self_test_result_buffer_index = 0;
        if (!tdata->testresult) {
            FTS_TEST_ERROR("test result is null");
            fts_self_test_result_buffer_index = snprintf(fts_self_test_result_buffer,
                TXT_BUFFER_LEN, "tdata or tdata->testresult is null\n");
        } else {
            if (tdata->testresult_len < TXT_BUFFER_LEN - 1) {
                fts_self_test_result_buffer_index = tdata->testresult_len;
            } else {
                fts_self_test_result_buffer_index = TXT_BUFFER_LEN - 1;
            }
            memcpy(fts_self_test_result_buffer, tdata->testresult, fts_self_test_result_buffer_index);
            fts_self_test_result_buffer[fts_self_test_result_buffer_index] = '\0';
        }
    }
...
static ssize_t fts_do_selftest_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    struct fts_ts_data *ts_data = dev_get_drvdata(dev);
    struct input_dev *input_dev = ts_data->input_dev;
    ssize_t size = 0;
    char fwname[FILE_NAME_LENGTH] = "thresholdfile_focaltech_xf908.ini";
    int ret = 0;
    int test_result = 0;

    FTS_DEBUG("selftest start");

    if (oplus_panel_id == 0x108003) {
        snprintf(fwname, FILE_NAME_LENGTH, "thresholdfile_focaltech_xh908.ini");
    } else if (oplus_panel_id == 0x097007) {
        snprintf(fwname, FILE_NAME_LENGTH, "thresholdfile_focaltech_xh906.ini");
    } else {
        snprintf(fwname, FILE_NAME_LENGTH, "thresholdfile_focaltech_xg908.ini");
    }
    ...
    //do test, code copy from fts_test_store()
    fts_irq_disable();
    fts_esdcheck_switch(ts_data, DISABLE);
    ...
    if (ret < 0) {
        ...
        test_result = fts_test_entry(fwname);
    }
    ret = fts_enter_test_environment(0);
    if (ret < 0) {
        FTS_ERROR("enter normal environment fail");
    ...
    //output test result
    if (test_result == 0) {
        size = snprintf(buf, PAGE_SIZE, "Status 0\n");
    } else if (test_result == 1) {
        size = snprintf(buf, PAGE_SIZE, "Status 1\n");
    } else {
        size = snprintf(buf, PAGE_SIZE, "Status 6\nFinish Cm/Cp test! Range Check Failure\n");
    }
    mutex_unlock(&input_dev->mutex);

    if (test_result == 1) {
        FTS_DEBUG("selftest end, pass!");
    } else {
        FTS_DEBUG("selftest end, fail, result = %d", test_result);
    }

    return size;
}

static ssize_t fts_selftest_result_show(struct device *dev,
				     ...
				     char *p_i8_buf)
{
    ...
    if (fts_self_test_result_buffer != NULL) {
        memcpy(p_i8_buf, fts_self_test_result_buffer, fts_self_test_result_buffer_index);
        p_i8_buf[fts_self_test_result_buffer_index] = '\0';
        return fts_self_test_result_buffer_index + 1;
    } else {
        p_i8_buf[0] = '\0';
        return 1;
    }
...
static DEVICE_ATTR(fts_do_selftest, S_IRUGO, fts_do_selftest_show, NULL);
static DEVICE_ATTR(fts_selftest_result, S_IRUGO, fts_selftest_result_show, NULL);

static struct attribute *fts_test_attributes[] = {
    &dev_attr_fts_test.attr,
    &dev_attr_fts_do_selftest.attr,
    &dev_attr_fts_selftest_result.attr,
    NULL
};
