...
*
* File Name: Focaltech_test_ft2389.c
*
...
*****************************************************************************/
static int ft2389_short_test(struct fts_test *tdata, bool *test_result)
{
    int ret = 0;
    int i = 0;
    ...
    for (i = 0; i < tdata->node.node_num; i++) {
        tmp_adc = adcdata[i];
        if ((tmp_adc >= 1) && (tmp_adc <= 914)) {
            tmp_adc = 914;
        }
        adcdata[i] = (129 * tmp_adc + 23552) / (2048 - 2 * tmp_adc);
    }
...
static int ft2389_open_test(struct fts_test *tdata, bool *test_result)
{
    int ret = 0;
    int i = 0;
    bool tmp_result = false;
    ...
    ret = fts_test_write_reg(FACTORY_REG_CB_ADDR_H, 0x0);
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("start open test fail\n");
        goto test_err;
    }

    ret = fts_test_write_reg(FACTORY_REG_CB_ADDR_L, 0x0);
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("start open test fail\n");
        goto test_err;
    }
...
static int ft2389_cb_test(struct fts_test *tdata, bool *test_result)
{
    int ret = 0;
    bool tmp_result = false;
    int byte_num = 0;
...
static int ft2389_rawdata_test(struct fts_test *tdata, bool *test_result)
{
    int ret = 0;
    bool tmp_result = false;
    int *rawdata = NULL;
    ...
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("write FACTORY_REG_DATA_SELECT fail\n");
        goto test_err;
    }
...
static int ft2389_lcdnoise_test(struct fts_test *tdata, bool *test_result)
{
    int ret = 0;
    int i = 0;
    bool tmp_result = false;
...
static int start_test_ft2389(void)
{
    int ret = 0;
    ...
    if (true == test_item->short_test) {
        ret = ft2389_short_test(tdata, &temp_result);
    ...
    if (true == test_item->open_test) {
        ret = ft2389_open_test(tdata, &temp_result);
        if ((ret < 0) || (false == temp_result)) {
            test_result = false;
        }
    }
    ...
    if (true == test_item->cb_test) {
        ret = ft2389_cb_test(tdata, &temp_result);
        if ((ret < 0) || (false == temp_result)) {
            test_result = false;
        }
    }
    ...
    if (true == test_item->rawdata_test) {
        ret = ft2389_rawdata_test(tdata, &temp_result);
        if ((ret < 0) || (false == temp_result)) {
            test_result = false;
        }
    }
    ...
    if (true == test_item->lcdnoise_test) {
        ret = ft2389_lcdnoise_test(tdata, &temp_result);
        if ((ret < 0) || (false == temp_result)) {
            test_result = false;
        }
    }
...
struct test_funcs test_func_ft2389 = {
    .ctype = {0x27},
    .hwtype = IC_HW_INCELL,
    .key_num_total = 0,
    .start_test = start_test_ft2389,
};
