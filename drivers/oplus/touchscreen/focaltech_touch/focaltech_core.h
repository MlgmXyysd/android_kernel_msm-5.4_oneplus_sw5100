...
*****************************************************************************/
struct Oplus_fts_point_record {
    int x;      /*x coordinate */
    int y;      /*y coordinate */
    int use;   /* 0 not use, 1 use */
};
...
struct fts_ts_data {
    ...
    struct work_struct exit_aod_work;
    bool shutdown_mode;
    bool nfc_reader_mode;
    bool easy_wakeup_state;
    int screen_state;
    int jitter_threshold;
    struct notifier_block switch_nb;
    //only if (irq_func && irq_switch && irq_sys == true) then enable irq
    bool irq_func;  //driver function need irq
    bool irq_switch;  //tp switch is on
    bool irq_sys;  //not in deepsleep or other system state
};
