...
#include <linux/oplus/kernel_fb.h>
...
unsigned int oplus_fts_ts_easy_wakeup_enable = 0;
EXPORT_SYMBOL(oplus_fts_ts_easy_wakeup_enable);
unsigned int oplus_fts_ts_aod_wakeup_enable = 0;
unsigned int oplus_fts_ts_aod_mode = 0;

extern unsigned int display_vflip;
#define IRQ_REPORT_EASYWAKEUP_MAX_TIME 3
int irq_report_easywakeup = 0;
#define PRINT_EASYWAKEUP_INFO_MAX_TIME 5
int print_easywakeup_info = 0;
...
static int oplus_fts_ts_exit_aod(struct device *dev);

void oplus_fts_force_recovery_(void);
void display_pwr_down_try_recovery(void);
...
int fts_reset_down(int hdelayms)
{
    FTS_DEBUG("tp reset down");
    gpio_direction_output(fts_data->pdata->reset_gpio, 0);
    if (hdelayms) {
        msleep(hdelayms);
    }
...
void fts_irq_disable_handle(void)
{
...
void fts_irq_enable_handle(void)
{
...
void check_irq_state(void) {
    bool new_irq_state = (fts_data->irq_func) &&
                (fts_data->irq_switch) && (fts_data->irq_sys);
    bool old_irq_state = !(fts_data->irq_disabled);

    FTS_DEBUG("tp_irq_enable %d->%d, <%d,%d,%d>", old_irq_state, new_irq_state,
        fts_data->irq_func, fts_data->irq_switch, fts_data->irq_sys);

    if (old_irq_state != new_irq_state) {
        if (new_irq_state) {
            fts_irq_enable_handle();
        } else {
            fts_irq_disable_handle();
        }
    }
}
...
{
    fts_data->irq_func = false;
    check_irq_state();
}

void fts_irq_enable(void)
{
    fts_data->irq_func = true;
    check_irq_state();
}

void fts_irq_switch(bool on)
{
    fts_data->irq_switch = on;
    check_irq_state();
}

void fts_irq_sys(bool on)
{
    fts_data->irq_sys = on;
    check_irq_state();
}
...
#if FTS_MT_PROTOCOL_B_EN
#define MAX_POINT_RECORD_NUM 10
struct Oplus_fts_point_record oplus_fts_point_record[MAX_POINT_RECORD_NUM] = {0};
...
static int fts_input_report_b(struct fts_ts_data *ts_data, struct ts_event *events)
{
    ...
    struct input_dev *input_dev = ts_data->input_dev;
    bool need_report_new_position = true;  //for anti-shake
    bool need_report_event = !oplus_fts_ts_aod_wakeup_enable;  //No points are reported in AOD mode
    ...
    for (i = 0; i < ts_data->touch_event_num; i++) {
        ...
        if (EVENT_DOWN(events[i].flag)) {
            need_report_new_position = true;
            if (FTS_TOUCH_DOWN == events[i].flag) {
                if (events[i].id < MAX_POINT_RECORD_NUM) {
                    oplus_fts_point_record[events[i].id].use = 1;
                    oplus_fts_point_record[events[i].id].x = events[i].x;
                    oplus_fts_point_record[events[i].id].y = events[i].y;
                }
            } else {
                if (events[i].id < MAX_POINT_RECORD_NUM) {
                    if (oplus_fts_point_record[events[i].id].use) {
                        if (((oplus_fts_point_record[events[i].id].x - events[i].x) * (oplus_fts_point_record[events[i].id].x - events[i].x)
                            + (oplus_fts_point_record[events[i].id].y - events[i].y) * (oplus_fts_point_record[events[i].id].y - events[i].y))
                            <= (ts_data->jitter_threshold * ts_data->jitter_threshold)) {
                            need_report_new_position = false;
                        } else {
                            oplus_fts_point_record[events[i].id].use = 0;
                        }
                    }
                }
            }

            if (need_report_event) {

                input_mt_slot(input_dev, events[i].id);
                input_mt_report_slot_state(input_dev, MT_TOOL_FINGER, true);
                if (need_report_new_position) {
    #if FTS_REPORT_PRESSURE_EN
                    input_report_abs(input_dev, ABS_MT_PRESSURE, events[i].p);
    #endif
                    input_report_abs(input_dev, ABS_MT_TOUCH_MAJOR, events[i].area);
                    if (display_vflip) {
                        input_report_abs(input_dev, ABS_MT_POSITION_X, ts_data->pdata->x_max - events[i].x + ts_data->pdata->x_min);
                        input_report_abs(input_dev, ABS_MT_POSITION_Y, ts_data->pdata->y_max - events[i].y + ts_data->pdata->y_min);
                    } else {
                    ...
                    if (display_vflip) {
                        input_report_abs(input_dev, ABS_MT_POSITION_X, ts_data->pdata->x_max - oplus_fts_point_record[events[i].id].x + ts_data->pdata->x_min);
                        input_report_abs(input_dev, ABS_MT_POSITION_Y, ts_data->pdata->y_max - oplus_fts_point_record[events[i].id].y + ts_data->pdata->y_min);
                    } else {
                        input_report_abs(input_dev, ABS_MT_POSITION_X, oplus_fts_point_record[events[i].id].x);
                        input_report_abs(input_dev, ABS_MT_POSITION_Y, oplus_fts_point_record[events[i].id].y);
                    }
                }
            }
    ...
    if (unlikely(touch_point_pre ^ touch_down_point_cur)) {
        for (i = 0; i < max_touch_num; i++)  {
            if ((1 << i) & (touch_point_pre ^ touch_down_point_cur)) {
                ...
                if (i < MAX_POINT_RECORD_NUM) {
                    oplus_fts_point_record[i].use = 0;
                }
            }
        }
    }
    ...
    if (touch_down_point_cur) {
        ...
        for (i = 0; i < MAX_POINT_RECORD_NUM; i++)  {
            oplus_fts_point_record[i].use = 0;
        }
        if (oplus_fts_ts_aod_wakeup_enable) {
            // run this when in aod wakeup state.
            input_report_key(ts_data->input_dev, KEY_F12, 1);
            input_report_key(ts_data->input_dev, KEY_F12, 0);
            input_sync(ts_data->input_dev);
            FTS_INFO("==touch to wakeup KEY_F12 == for aod wakeup");
        }
    }
...
static int fts_read_parse_touchdata(struct fts_ts_data *ts_data, u8 *touch_buf)
{
    ...
    if (touch_buf[0] == FTS_PLAM_FLAG) {
        return TOUCH_LARGE_HAND;
    }
...
static int fts_irq_read_report(struct fts_ts_data *ts_data)
{
    ...
    switch (touch_etype) {
    case TOUCH_LARGE_HAND:
        input_report_key(ts_data->input_dev, KEY_SLEEP, 1);
        input_report_key(ts_data->input_dev, KEY_SLEEP, 0);
        input_sync(ts_data->input_dev);
        FTS_INFO("Tp large trigger KEY_SLEEP");
        break;
    case TOUCH_DEFAULT:
...
static irqreturn_t fts_irq_handler(int irq, void *data)
{
    ...
    if (ts_data->pm_suspend) {
        if (oplus_fts_ts_aod_wakeup_enable) {
            ...
            if (!ret) {
                ...
                input_report_key(ts_data->input_dev, KEY_F12, 0);
                input_sync(ts_data->input_dev);
                FTS_INFO("== touch to wakeup KEY_F12 == for aod skip pm suspend");
                return IRQ_HANDLED;
            }
        } else if (ts_data->easy_wakeup_state) {
            if (irq_report_easywakeup > 0) {
                irq_report_easywakeup--;
                input_report_key(ts_data->input_dev, KEY_F12, 1);
                input_report_key(ts_data->input_dev, KEY_F12, 0);
                input_sync(ts_data->input_dev);
                FTS_INFO("== touch to wakeup KEY_F12 == for easy wakeup skip pm suspend");
                return IRQ_HANDLED;
            }
        }
    }
    ...
    if (ts_data->easy_wakeup_state) {
        // run this when in easy wakeup state.
        ret = fts_read_reg(FTS_REG_GESTURE_OUTPUT_ADDRESS, &flag_gesture);
        if (ret < 0) {
            FTS_ERROR("read FTS_REG_GESTURE_OUTPUT_ADDRESS fail, ret = %d", ret);
            return IRQ_HANDLED;
        }

        if (flag_gesture == FTS_GUESTURE_CLICK) {
            input_report_key(ts_data->input_dev, KEY_F12, 1);
            input_report_key(ts_data->input_dev, KEY_F12, 0);
            input_sync(ts_data->input_dev);
            FTS_INFO("== touch to wakeup KEY_F12 == for easy wakeup");
            return IRQ_HANDLED;
        }

        //abnormal state
        ret = fts_read_reg(FTS_REG_GESTURE_EN, &flag_gesture_en);
        if (ret < 0) {
            flag_gesture_en = -1;
        }
        ret = fts_read_reg(FTS_REG_PALM_OUTPUT_ADDRESS, &flag_palm);
        if (ret < 0) {
            flag_palm = -1;
        }

        if (flag_gesture_en == 0 && flag_palm == FTS_PLAM_FLAG) {
            FTS_INFO("ignore large hand event in easy wakeup state");
        } else if (ts_data->screen_state == FTS_SCREEN_STATE_OFF) {
            if (irq_report_easywakeup > 0) {
                irq_report_easywakeup--;
                input_report_key(ts_data->input_dev, KEY_F12, 1);
                input_report_key(ts_data->input_dev, KEY_F12, 0);
                input_sync(ts_data->input_dev);
                FTS_INFO("== touch to wakeup KEY_F12 == for abnormal state [%d, %d, %d]",
                flag_gesture, flag_gesture_en, flag_palm);
            }
        } else {
            if (print_easywakeup_info > 0) {
                print_easywakeup_info--;
                FTS_INFO("ignore irq with reg [%d, %d, %d] in other screen state (%d) time %d",
                    flag_gesture, flag_gesture_en, flag_palm, ts_data->screen_state, print_easywakeup_info);
            }
        }
        return IRQ_HANDLED;
    }
...
static void oplus_fts_ts_exit_aod_work(struct work_struct *work)
{
    struct fts_ts_data *ts_data = container_of(work, struct fts_ts_data,
                                  exit_aod_work);

    oplus_fts_ts_exit_aod(ts_data->dev);
}
...
#else
static void gesture_slide_enable(bool en)
{
    if (en) {
        fts_write_reg(FTS_REG_GESTURE_EN_SLIDE, 1);
    } else {
        fts_write_reg(FTS_REG_GESTURE_EN_SLIDE, 0);
    }
    return;
}
                                 ...
                                 unsigned long event, void *data)
{
    ...
    irq_report_easywakeup = IRQ_REPORT_EASYWAKEUP_MAX_TIME;

    if (event == MSM_DRM_EVENT_IDLE) {
        blank = evdata->data;
        FTS_INFO("DRM IDLE EVENT, blank:%d", *blank);
        if (*blank == MSM_DRM_ENTER_IDLE) {
            ts_data->screen_state = FTS_SCREEN_STATE_OFF;
            cancel_work_sync(&fts_data->resume_work);
            oplus_fts_ts_aod_mode = 1;
            fts_ts_suspend(ts_data->dev);
            oplus_fts_ts_aod_wakeup_enable = 0;
        } else if (*blank == MSM_DRM_EXIT_IDLE) {
            ts_data->screen_state = FTS_SCREEN_STATE_ON;
            oplus_fts_ts_aod_mode = 0;
            queue_work(fts_data->ts_workqueue, &fts_data->resume_work);
        }
        return 0;
    }
    ...
    print_easywakeup_info = PRINT_EASYWAKEUP_INFO_MAX_TIME;
    ...
    case MSM_DRM_BLANK_UNBLANK:
        if (MSM_DRM_EARLY_EVENT_BLANK == event) {
            ts_data->screen_state = FTS_SCREEN_STATE_OFF_TO_ON;
            FTS_INFO("resume: event = %lu, not care\n", event);
    ...
    case MSM_DRM_BLANK_POWERDOWN:
        if (MSM_DRM_EARLY_EVENT_BLANK == event) {
            ts_data->screen_state = FTS_SCREEN_STATE_ON_TO_OFF;
            cancel_work_sync(&fts_data->resume_work);
            oplus_fts_ts_aod_mode = 0;
            fts_ts_suspend(ts_data->dev);
            oplus_fts_ts_aod_wakeup_enable = 0;
...
static int switch_notifier_callback(struct notifier_block *self,
    ...
    if(switch_states == NULL) {
        FTS_ERROR("switch_states is null");
        return 0;
    }

    if((event == SWITCH_PRE_ACTION) && (switch_states[1] == SWITCH_DISCONNECT)) {
        //switch to mcu
        fts_irq_switch(false);
    } else if((event == SWITCH_POST_ACTION) && (switch_states[1] != SWITCH_DISCONNECT)) {
        //switch to ap
        fts_irq_switch(true);
    }
...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
    ...
    if (ret) {
        FTS_ERROR("not focal IC, unregister driver, retry");
        fts_reset_proc(200);
        ret = fts_get_ic_information(ts_data);
    }
    if (ret) {
        FTS_ERROR("not focal IC, unregister driver, finally");
        //goto err_irq_req;
        ts_data->ic_info.ids.chip_idh = 0x23;
        ts_data->ic_info.ids.chip_idl = 0x89;
        ts_data->ic_info.cid.type = 0x0;
    }
    ...
    if (ts_data->ts_workqueue) {
        ...
        INIT_WORK(&ts_data->exit_aod_work, oplus_fts_ts_exit_aod_work);
    }
    ...
    ts_data->switch_nb.notifier_call = switch_notifier_callback;
    ret = oplus_switch_notifier_register(SWITCH_TP, ts_data->dev, &ts_data->switch_nb);
    if (ret) {
        FTS_ERROR("oplus_switch_notifier_register failed, ret=%d", ret);
    }
...
static int fts_ts_remove_entry(struct fts_ts_data *ts_data)
{
    ...
    if (oplus_switch_notifier_unregister(SWITCH_TP, ts_data->dev, &ts_data->switch_nb) != 0) {
        FTS_ERROR("oplus_switch_notifier_unregister failed");
    }
...
static int fts_ts_suspend(struct device *dev)
{
    ...
    if (oplus_fts_ts_aod_mode) {
        FTS_INFO("make TP enter into aod wakeup mode");
        fts_gesture_suspend(ts_data);
        gesture_slide_enable(true);
        ts_data->easy_wakeup_state = true;
    } else if (oplus_fts_ts_easy_wakeup_enable) {
        FTS_INFO("make TP enter into easy wakeup mode");
        fts_gesture_suspend(ts_data);
        gesture_slide_enable(false);
        ts_data->easy_wakeup_state = true;
    } else {
        fts_irq_disable();
...
{
    struct fts_ts_data *ts_data = fts_data;
    int wait_tp_to_valid = 0;
    static int force_recovery_ic_time = 5;
    ...
    wait_tp_to_valid = fts_wait_tp_to_valid();
    fts_ex_mode_recovery(ts_data);

    fts_esdcheck_resume(ts_data);

    if (ts_data->easy_wakeup_state) {
        FTS_DEBUG("Get out of easy wakeup state");
        ts_data->easy_wakeup_state = false;
        fts_gesture_resume(ts_data);
    } else {
        FTS_DEBUG("Get out of sleep mode");
        //rewrite charger mode
        if(fts_write_reg(FTS_REG_CHARGER_MODE_EN, ts_data->charger_mode) < 0) {
            FTS_ERROR("write charge mode failed");
        }
    }
    fts_irq_enable();

    if(wait_tp_to_valid != 0) {
        if (force_recovery_ic_time > 0) {
            msleep(1000);
            FTS_ERROR("fts_wait_tp_to_valid() fail, force recovery for the last %d times", force_recovery_ic_time);
            force_recovery_ic_time--;
            oplus_fts_force_recovery_();
        } else if (force_recovery_ic_time == 0) {
            force_recovery_ic_time--;
            FTS_ERROR("force recovery TP fail");
            oplus_kevent_fb(FB_TP, "tp_invalid", "force recovery Focal TP fail");
        }
    }
...
void oplus_fts_force_power_off_(struct device *dev) {
    FTS_FUNC_ENTER();
    oplus_fts_ts_easy_wakeup_enable = 0;
    cancel_work_sync(&fts_data->resume_work);
    fts_ts_suspend(dev);
    FTS_FUNC_EXIT();
}

void oplus_fts_force_recovery_() {
    int wait_tp_to_valid = 0;
    FTS_FUNC_ENTER();
    fts_irq_disable();
    fts_reset_proc(200);
    wait_tp_to_valid = fts_wait_tp_to_valid();
    if(wait_tp_to_valid == 0) {
        FTS_DEBUG("recovery by reset");
        fts_irq_enable();
    } else {
        FTS_DEBUG("try to recovery by power down");
        display_pwr_down_try_recovery();
    }
    FTS_FUNC_EXIT();
}
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    ts_data->shutdown_mode = 0;
    ts_data->nfc_reader_mode = 0;
    ts_data->easy_wakeup_state = 0;
    ts_data->screen_state = FTS_SCREEN_STATE_ON;
    ts_data->jitter_threshold = 15;
    ts_data->irq_func = true;
    ts_data->irq_switch = true;
    ts_data->irq_sys = true;
...
static int __init fts_ts_init(void)
{
    ...
    if (!(oplus_panel_id == 0x108002 || oplus_panel_id == 0x108003 || oplus_panel_id == 0x097007)) {
        return 0;
    }
