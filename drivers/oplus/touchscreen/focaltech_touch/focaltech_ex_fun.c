    ...
    struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
    ...
    if (buflen + 1 > PROC_BUF_SIZE) {
        writebuf = (u8 *)kzalloc((buflen + 1) * sizeof(u8), GFP_KERNEL);
        if (NULL == writebuf) {
    ...
    ret = buflen;
proc_write_err:
    if ((buflen + 1 > PROC_BUF_SIZE) && writebuf) {
        kfree(writebuf);
        writebuf = NULL;
    }
    return ret;
...
extern void oplus_fts_force_power_off_(struct device *dev);
/*
 * power off via sysfs
 */
static ssize_t oplus_fts_force_power_off_store(struct device *dev,
    struct device_attribute *attr, const char *buf, size_t size)
{
    FTS_FUNC_ENTER();
    oplus_fts_force_power_off_(dev);
    FTS_FUNC_EXIT();
    return size;
}

/* Read fw verion to check connection with TP IC */
static ssize_t fts_check_connection_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    int ret = 0;
    struct fts_ts_data *ts_data = dev_get_drvdata(dev);
    ...
    if ((ret < 0) || (fwver == 0xFF) || (fwver == 0x00)) {
        FTS_ERROR("%s: get tp fw version fail!\n", __func__);
        num_read_chars = snprintf(buf, PAGE_SIZE, "1\n");
    } else {
        if (fwver >= 0x60 && fwver <= 0xB0) {
            FTS_DEBUG("%s: connected, fwver = %d\n", __func__, fwver);
            num_read_chars = snprintf(buf, PAGE_SIZE, "0\n");
        } else {
            FTS_ERROR("%s: disconnected, fwver = %d\n", __func__, fwver);
            num_read_chars = snprintf(buf, PAGE_SIZE, "1\n");
        }
    }
...
/* fts_jitter_threshold node */
static ssize_t fts_jitter_threshold_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    int count = 0;
    struct fts_ts_data *ts_data = dev_get_drvdata(dev);
    ...
    mutex_lock(&input_dev->mutex);
    count += snprintf(buf + count, PAGE_SIZE, "jitter threshold:%d\n",
                      ts_data->jitter_threshold);
    mutex_unlock(&input_dev->mutex);

    return count;
}

static ssize_t fts_jitter_threshold_store(
    struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
    int value = 0;
    ...
    FTS_FUNC_ENTER();
    mutex_lock(&input_dev->mutex);
    sscanf(buf, "%20d", &value);
    FTS_DEBUG("jitter threshold:%d->%d", ts_data->jitter_threshold, value);
    ts_data->jitter_threshold = value;
    mutex_unlock(&input_dev->mutex);
    FTS_FUNC_EXIT();
...
static DEVICE_ATTR(fts_force_power_off, S_IWUSR | S_IWGRP, NULL, oplus_fts_force_power_off_store);
static DEVICE_ATTR(fts_check_connection, S_IRUGO, fts_check_connection_show, NULL);
static DEVICE_ATTR(fts_jitter_threshold, S_IRUGO | S_IWUSR, fts_jitter_threshold_show, fts_jitter_threshold_store);
...
static struct attribute *fts_attributes[] = {
    &dev_attr_fts_force_power_off.attr,
    &dev_attr_fts_check_connection.attr,
    &dev_attr_fts_jitter_threshold.attr,
