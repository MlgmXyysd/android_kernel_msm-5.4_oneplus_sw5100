    ...
    struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (ts_data->shutdown_mode == ENABLE) {
        FTS_ERROR("in shutdown mode, not support change charger mode");
        return count;
    }
    if (ts_data->fw_loading) {
        FTS_ERROR("fw upgrade in process, not support change charger mode");
        return count;
    }
...
static ssize_t fts_shutdown_mode_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    int count = 0;
    struct fts_ts_data *ts_data = dev_get_drvdata(dev);
    ...
    count = snprintf(buf, PAGE_SIZE, "%d\n",
        ts_data->shutdown_mode);

    return count;
}

static ssize_t fts_shutdown_mode_store(
    struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
    int ret = 0;
    ...
    if (FTS_SYSFS_ECHO_ON(buf)) {
        FTS_DEBUG("enter shutdown mode");
        ret = fts_ex_mode_switch(MODE_SHUTDOWN, ENABLE);
        ts_data->shutdown_mode = ENABLE;
    } else if (FTS_SYSFS_ECHO_OFF(buf)) {
        if (ts_data->shutdown_mode == DISABLE) {
            if(fts_wait_tp_to_valid() == 0) {
                FTS_DEBUG("not in shutdown mode");
                return count;
            }
        }
        FTS_DEBUG("exit shutdown mode");
        ret = fts_ex_mode_switch(MODE_SHUTDOWN, DISABLE);
        ts_data->shutdown_mode = DISABLE;
    }

    FTS_DEBUG("shutdown mode:%d", ts_data->shutdown_mode);
    return count;
}

static ssize_t fts_nfc_reader_mode_show(
    struct device *dev, struct device_attribute *attr, char *buf)
{
    int count = 0;
    struct fts_ts_data *ts_data = dev_get_drvdata(dev);
    ...
    count = snprintf(buf, PAGE_SIZE, "%d\n",
        ts_data->nfc_reader_mode);

    return count;
}

static ssize_t fts_nfc_reader_mode_store(
    struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
    struct fts_ts_data *ts_data = dev_get_drvdata(dev);
    ...
    if (FTS_SYSFS_ECHO_ON(buf)) {
        FTS_DEBUG("enter nfc reader mode");
        ts_data->nfc_reader_mode = ENABLE;
    } else if (FTS_SYSFS_ECHO_OFF(buf)) {
        if (ts_data->nfc_reader_mode == ENABLE) {
            FTS_DEBUG("exit nfc reader mode, reset");
            mutex_lock(&input_dev->mutex);
            fts_irq_disable();
            fts_release_all_finger();
            fts_reset_proc(100);
            ...
            FTS_DEBUG("already exit nfc reader mode");
        }
        ts_data->nfc_reader_mode = DISABLE;
    }

    FTS_DEBUG("nfc reader mode:%d", ts_data->nfc_reader_mode);
    return count;
}
...
static DEVICE_ATTR(fts_shutdown_mode, S_IRUGO | S_IWUSR | S_IWGRP,
                   fts_shutdown_mode_show, fts_shutdown_mode_store);

static DEVICE_ATTR(fts_nfc_reader_mode, S_IRUGO | S_IWUSR | S_IWGRP,
                   fts_nfc_reader_mode_show, fts_nfc_reader_mode_store);

static struct attribute *fts_touch_mode_attrs[] = {
    &dev_attr_fts_glove_mode.attr,
    &dev_attr_fts_cover_mode.attr,
    &dev_attr_fts_charger_mode.attr,
    &dev_attr_fts_shutdown_mode.attr,
    &dev_attr_fts_nfc_reader_mode.attr,
    NULL,
};
...
int fts_ex_mode_recovery(struct fts_ts_data *ts_data)
{
    ...
    if (ts_data->charger_mode) {
        FTS_DEBUG("recovery enter charger mode");
        fts_ex_mode_switch(MODE_CHARGER, ENABLE);
    } else {
        FTS_DEBUG("recovery exit charger mode");
        fts_ex_mode_switch(MODE_CHARGER, DISABLE);
    }
