...
extern unsigned int is_recovery_mode;
...
static ssize_t shutdown_detect_trigger(struct file *filp, const char *ubuf, size_t cnt, loff_t *data)
{
    ...
    pr_info("(%ld) %s:%d shutdown_detect GroupLeader is %s:%d\n", current_kernel_time().tv_sec,
    ...
    case SHUTDOWN_STAGE_KERNEL:
        oplus_batt_soc_set(ui_soc);
        if(oplus_enable_ship_mode_flag) {
            oplus_enter_shipmode();
        }
        shutdown_kernel_start_time = current_kernel_time().tv_sec;
        //Ansheng Ma@OPLUS.com Android.Framework.Stability every reboot create shutdownlog fatal
