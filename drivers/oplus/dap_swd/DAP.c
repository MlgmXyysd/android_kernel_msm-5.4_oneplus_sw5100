...
//#define DAP_FW_VER "1.20" // Firmware Version
...
static __inline void TIMER_START(uint32_t usec)
{
  ...
  TimerTick = (begin.tv_sec) * 1000000 + begin.tv_usec + usec;
}
...
static __inline uint32_t TIMER_EXPIRED(void)
{
  struct timeval end;
  do_gettimeofday(&(end));
  return (((end.tv_sec) * 1000000 + end.tv_usec) > TimerTick) ? 1U : 0U ;
}
...
static uint32_t DAP_SWJ_Clock(const uint8_t *request, uint8_t *response)
{
  ...
  DAP_Data.hz = clock ;
  DAP_Data.delay_ns = 1000 * 1000 * 1000 / clock / 2 ;
      ...
      if (post_read)
      {
        ...
        if ((request_value & (DAP_TRANSFER_APnDP | DAP_TRANSFER_MATCH_VALUE)) == DAP_TRANSFER_APnDP)
        {
          ...
          do
          {
            ...
            if(response_value == DAP_TRANSFER_WAIT){
                mdelay(10);
            }
...
void DAP_Setup(void)
{
...
#endif
  spin_lock_init(&DAP_Data.rw_lock);
  if (DAP_DEFAULT_SWJ_CLOCK >= MAX_SWJ_CLOCK(DELAY_FAST_CYCLES)) {
    ...
    delay = ((CPU_CLOCK/2U) + (DAP_DEFAULT_SWJ_CLOCK - 1U)) / DAP_DEFAULT_SWJ_CLOCK;
    if (delay > IO_PORT_WRITE_CYCLES) {
      delay -= IO_PORT_WRITE_CYCLES;
      delay  = (delay + (DELAY_SLOW_CYCLES - 1U)) / DELAY_SLOW_CYCLES;
    } else {
