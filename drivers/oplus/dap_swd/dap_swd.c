...
({ \
	...
	req |= parity32(req&0xF) << SWD_REQ_PARITY_BIT; \
...
#define REG_APOLLO4_SCRATCH0    0x400201C0
#define REG_APOLLO4_BOOTLDR     0x400201B8
#define REG_APOLLO4_JDEC_PID    0xF0000FE0

struct swd_ctx * swd_context = NULL;
static int swd_xfer_retry(struct swd_ctx *swd, uint8_t req, uint32_t *data);

extern int is_swd_rw;
/*
...
#define SWD_CLK_OUT_HI		writel_relaxed_no_log(MSM_GPIO_IO_OUT, swd->gpio_clk_reg.io_reg)
#define SWD_CLK_OUT_LO		writel_relaxed_no_log(0, swd->gpio_clk_reg.io_reg)

#define SWD_DATA_DIR_OUT	writel_relaxed_no_log(data_ctl | MSM_GPIO_CTL_OE, swd->gpio_data_reg.ctl_reg)
#define SWD_DATA_DIR_IN		writel_relaxed_no_log(data_ctl & ~MSM_GPIO_CTL_OE, swd->gpio_data_reg.ctl_reg)
#define SWD_DATA_OUT(x)		writel_relaxed_no_log((x) ? MSM_GPIO_IO_OUT : 0, swd->gpio_data_reg.io_reg)
#define SWD_DATA_IN			(readl_relaxed_no_log(swd->gpio_data_reg.io_reg) & MSM_GPIO_IO_IN)
/*#define SWD_DATA_DIR_OUT	gpio_direction_output(gpio_data,1)
#define SWD_DATA_DIR_IN		gpio_direction_input(gpio_data)
#define SWD_DATA_OUT(x)		gpio_set_value(gpio_data,x)
#define SWD_DATA_IN			gpio_get_value(gpio_data)*/
...
static inline int swd_write_aircr(struct swd_ctx *swd, uint32_t addr, uint32_t data)
{
	uint8_t req;
	int ack;
...
static int swd_dap_pinctrl_configure(struct swd_ctx *swd, bool active)
{
	struct pinctrl_state *state;
	int ret;

	state = pinctrl_lookup_state(swd->pinctrl, active ? "dap_active" : "default");
	if (IS_ERR(state)) {
		dev_err(swd->dev, "cannot get pinctrl %s state\n", active ? "active" : "default");
		return PTR_ERR(state);
	}
...
static int swd_init_debug(struct swd_ctx *swd)
{
	...
	ack = swd_write_dp(swd, DP_CTRL_STAT, CSYSPWRUPREQ | CDBGPWRUPREQ | STICKYERR);
	if (ack != SWD_ACK_OK)
		return ack;
...
static void swd_disconnect(struct swd_ctx *swd)
{
	...
	swd->halted = false;
	swd->connected = false;
	swd->halt_frozen = false;
}

static void swd_power_off_mcu(struct swd_ctx *swd)
{
	swd_write_word(swd, DBG_HCSR, DBGKEY);
	swd_write_dp(swd, DP_CTRL_STAT, 0);
	...
	swd_to_jtag(swd);
	swd_pinctrl_configure(swd, false);
	oplus_sensorhub_power_off_once(swd->dev);
...
static int swd_reset_halt(struct swd_ctx *swd)
{
	...
	if (swd->soft_reset) {
		...
		do {
			...
			mdelay(20);
			ap_retries++;
		//if not halted, timeout 500ms, jump out this loop.
		//if halted, jump out this loop.
		} while (((tmp & S_HALT) == 0) && (ap_retries < 25));

		tmp = 0;
		ret = swd_read_word(swd, REG_APOLLO4_JDEC_PID, &tmp);
		if (ret != SWD_ACK_OK)
			return swd_ack_to_errno(ret);

		if ((tmp & 0xF0) == 0xB0) {
			ret = swd_read_word(swd, REG_APOLLO4_BOOTLDR, &tmp);
			if (ret != SWD_ACK_OK)
				return swd_ack_to_errno(ret);
		}

		if ((tmp & 0x0C000000) == 0x04000000) {
			dev_info(swd->dev, "apollo4 has sbl:0x%x\n", tmp);
			ret = swd_read_word(swd, REG_APOLLO4_SCRATCH0, &tmp);
			if (ret != SWD_ACK_OK)
				return swd_ack_to_errno(ret);

			ret = swd_write_word(swd, REG_APOLLO4_SCRATCH0, tmp | 1);
			if (ret != SWD_ACK_OK)
				return swd_ack_to_errno(ret);
		} else {
			dev_info(swd->dev, "apollo4 has no sbl:0x%x\n", tmp);
			// Enable halt on reset
			ret = swd_write_word(swd, DBG_EMCR, TRCENA | VC_CORERESET);
			if (ret != SWD_ACK_OK)
				return swd_ack_to_errno(ret);
		}
	...
	do {
		ret = swd_read_word(swd, DBG_HCSR, &tmp);
		if (ap_retries >= 5) {
			dev_info(swd->dev, "Timeout while waiting for CPU to halt after reset, reinit swd interface\n");
			swd_init_debug(swd);
			done = 1;
			timeout = 1;
		} else if ((tmp == 0xFFFFFFFF) || ((tmp & 0xFFFF0000)==0x05FA0000)) {
			dev_info(swd->dev, "resetting swd interface, tmp=0x%x\n", tmp);
			swd_init_debug(swd);
		//Bit 17: S_HALT in the DHCSR.
		} else if ((tmp != 0xFFFFFFFF) && (tmp & 0x00020000)) {
			dev_info(swd->dev, "CPU halted after reset\n");
			done = 1;
		}
		mdelay(100);
		ap_retries++;
	...
	ret = swd_write_word(swd, 0xE0002000, 0x00000003);
	if (ret != SWD_ACK_OK)
		return swd_ack_to_errno(ret);

	ret = swd_write_word(swd, 0xE000EDFC, 0x01000000);
	if (ret != SWD_ACK_OK)
		return swd_ack_to_errno(ret);

	//reset and halt is timeout, try to manually halt.
	if (timeout == 1) {
		// Enable debug and halt the core (DHCSR <- 0xA05F0003)
		ret = swd_write_word(swd, DBG_HCSR, DBGKEY | C_DEBUGEN | C_HALT);
		if (ret != SWD_ACK_OK)
		...
		do {
			...
			mdelay(20);
			if (++ap_retries >= 25) {
				dev_info(swd->dev, "Timeout while waiting for manually halt mcu\n");
				return -EFAULT;
			}
		} while (((tmp & S_HALT) == 0) || ((tmp & S_LOCKUP) == S_LOCKUP));
	}
...
static u8 in[DAP_PACKET_SIZE] ;
static u8 out[DAP_PACKET_SIZE];
		...
		unsigned int cmd, unsigned long arg)
{
	...
	case SWD_IOCTL_POWER_OFF_MCU:
		mutex_lock(&swd->lock);
		swd_power_off_mcu(swd);
		mutex_unlock(&swd->lock);
		break;
	...
	case SWD_IOCTL_RD_MEM:
		...
		ret = __put_user(u_args[1], (unsigned int *)(arg + sizeof(unsigned int)));
		break;
    case SWD_IOCTL_DAP_RW:
		ret = copy_from_user(u_args, (void __user *)arg, sizeof(u_args));
		if (ret)
			return ret;
		memset(in,0,DAP_PACKET_SIZE);
		memset(out,0,DAP_PACKET_SIZE);
		ret = copy_from_user(in, (void __user *)u_args[0], DAP_PACKET_SIZE);
		if (ret)
			return ret;
		memcpy(out,in,DAP_PACKET_SIZE);

		mutex_lock(&swd->lock);
		DAP_ExecuteCommand(in,out);
		mutex_unlock(&swd->lock);

		ret = copy_to_user((void __user *)u_args[1], out, DAP_PACKET_SIZE);
		if (ret)
			return ret;
		break ;
	case SWD_IOCTL_PIN_ON:
		swd_dap_pinctrl_configure(swd, true);
		ret = 0 ;
		break;
	case SWD_IOCTL_PIN_OFF:
		swd_dap_pinctrl_configure(swd, false);
		ret = 0 ;
		break;
	case SWD_IOCTL_HALT_FROZEN:
		mutex_lock(&swd->lock);
		if (swd->connected)
			ret = swd_halt(swd);
		else
			ret = -EINVAL;
		swd->halt_frozen = true;
		mutex_unlock(&swd->lock);
		break;
	default:
		return -EINVAL;
...
static ssize_t swd_show_pinctrl(struct device *dev,
			   struct device_attribute *attr, char *buf)
{
	struct swd_ctx *swd = dev_get_drvdata(dev);
	mutex_lock(&swd->lock);
...
static ssize_t swd_store_pinctrl(struct device *dev, struct device_attribute *attr,
			  const char *buf, size_t count)
{
	struct swd_ctx *swd = dev_get_drvdata(dev);
...
static ssize_t swd_show_data_io(struct device *dev,
			   struct device_attribute *attr, char *buf)
{
	struct swd_ctx *swd = dev_get_drvdata(dev);
	...
	return sprintf(buf, "io:%s\n", "fine");
}

static ssize_t swd_store_data_io(struct device *dev, struct device_attribute *attr,
			  const char *buf, size_t count)
{
	struct swd_ctx *swd = dev_get_drvdata(dev);
	...
	mutex_lock(&swd->lock);
	if(val){
		//dap_swdio_out_enable();
	}else{
		//dap_swdio_out_disable();
	}
	mutex_unlock(&swd->lock);
	return count;
}
...
static const DEVICE_ATTR(pinctrl, 0664, swd_show_pinctrl, swd_store_pinctrl);
static const DEVICE_ATTR(data_io, 0664, swd_show_data_io, swd_store_data_io);
//static const DEVICE_ATTR(clk_io, 0664, swd_show_register, swd_store_register);
#endif
...
static int swd_probe(struct platform_device *pdev)
{
	...
	ret = oplus_sensorhub_reset_notifier_register(&pdev->dev, &swd->rst_nb);
	if (ret < 0) {
	...
	if (ret) {
		oplus_sensorhub_reset_notifier_unregister(&pdev->dev, &swd->rst_nb);
...
MODULE_AUTHOR("Oplus");
MODULE_DESCRIPTION("OPLUS-SWD driver");
MODULE_LICENSE("GPL");
MODULE_ALIAS("platform: OPLUS-SWD");
