...
static int send_deep_sleep_vote(void)
{
	...
	if (gpio_request(ds_exit_gpio, "ds_exit_gpio")) {
		pr_err("ds_exit_gpio Already requested\n");
        }
        gpio_direction_output(ds_exit_gpio, 0);
        gpio_set_value(ds_exit_gpio, 0);
        pr_debug("%s gpio:%d set to low\n", __func__, ds_exit_gpio);
        udelay(1000);
        gpio_set_value(ds_exit_gpio, 1);
	pr_err(" send to RPM when enter deepsleep");
...
static int lpm_probe(struct platform_device *pdev)
{
	...
	if (!gpio_is_valid(ds_exit_gpio)) {
		pr_err("%s ds_exit_gpio request failed\n", __func__);
        }
