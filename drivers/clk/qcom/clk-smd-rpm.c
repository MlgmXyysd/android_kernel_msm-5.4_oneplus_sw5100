				...
				unsigned long parent_rate)
{
    ...
    if(r->rpm_clk_id == snoc_rpm_clk_id && is_swd_rw ){
		rate = 240000000;
		//pr_err("clk_smd_rpm_set_rate rpm_clk_id:%d %ld %u\n",r->rpm_clk_id,rate);
	}else if( r->rpm_clk_id == cnoc_rpm_clk_id && is_swd_rw ){
		rate = 75000000;
		//pr_err("clk_smd_rpm_set_rate rpm_clk_id:%d %ld %u\n",r->rpm_clk_id,rate);
	}
	#endif
...
static int rpm_smd_clk_probe(struct platform_device *pdev)
{
	...
	if (is_holi || is_monaco || is_scuba || is_bengal) {
		...
		cnoc_rpm_clk_id = holi_cnoc_a_clk.rpm_clk_id;
		snoc_rpm_clk_id = holi_snoc_a_clk.rpm_clk_id;
