...
int qbgsoc = -1;
int updatesoc = 0;
extern bool opti_uevt_t;
extern bool opti_uevt_v;
extern bool pre_opti_uevt;

extern int factoraging_control;
extern int ITERM_MA;
		...
		union power_supply_propval *val)
{
	...
	switch (psp) {
	case POWER_SUPPLY_PROP_USB_CURRENT:
...
static enum power_supply_property smblite_batt_props[] = {
	...
	POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT,
	POWER_SUPPLY_PROP_CHG_LIMIT,
	POWER_SUPPLY_PROP_SET_CHG_CURRENT,
	POWER_SUPPLY_PROP_DEMO_MODE,
	POWER_SUPPLY_PROP_CHARGING_ENABLED,
	POWER_SUPPLY_PROP_AGING_CHG_CTL,

};

extern int ui_soc;
extern bool soc_updated;
extern int hvdcp_changed;
int batt_status = 0;
static bool soc_inited = false;
extern void oplus_trigger_recharge(void);

void oplus_calibrate_ponsoc(int *ponsoc) {
	pr_info("ponqbg %d,lastsave %d, sts %d", *ponsoc, ui_soc, batt_status);

	if (batt_status == POWER_SUPPLY_STATUS_UNKNOWN)
		return;
...
void oplus_calibrate_fullsoc(struct smb_charger *chg, int *fullsoc) {
	if(charger_log_enable)
		pr_info("fullqbg %d,ui %d, sts %d", *fullsoc, ui_soc, batt_status);

	//needed to updated immediately if hvdcp update for booting
	if (hvdcp_changed == 1) {
		hvdcp_changed++;
		ui_soc = *fullsoc;
		return;
	...
	if ((batt_status != POWER_SUPPLY_STATUS_CHARGING)) {
		if ((BATVOL <= UPPER_LIMIT_OF_ULTRA_LOW_VOLTAGE_UV) && (ui_soc >= 0)) {
			__pm_stay_awake(chg->smooth_soc);
			schedule_delayed_work(&chg->soc_work, msecs_to_jiffies(5000));//5s

			if (BATVOL <= LOWER_LIMIT_OF_ULTRA_LOW_VOLTAGE_UV) {
				--ui_soc;
			}
			if (ui_soc < 0) {
				ui_soc = 0;
			...
			*fullsoc = ui_soc;
			...
			ui_soc = *fullsoc = min(*fullsoc, ui_soc);
		}
	} else {
		__pm_relax(chg->smooth_soc);
		ui_soc = *fullsoc;
		...
		union power_supply_propval *val)
{
	...
	switch (psp) {
	case POWER_SUPPLY_PROP_CHG_LIMIT:
		val->intval = chg->chg_limit;
	break;
	case POWER_SUPPLY_PROP_SET_CHG_CURRENT:
		val->intval = chg->chg_current_limit;
	break;
	case POWER_SUPPLY_PROP_DEMO_MODE:
		val->intval = chg->demo_mode;
	break;
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		val->intval = chg->charging_enabled;
	break;
	case POWER_SUPPLY_PROP_AGING_CHG_CTL:
		if (!aging_ctl_inited) {
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		//only fulled then can opti healthd uevent
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		//as. 50->52; avoid ultralow vol,and vol rebound
		if ((hvdcp_changed==2) && ((val->intval - ui_soc) > 1) && \
				(BATVOL > (UPPER_LIMIT_OF_ULTRA_LOW_VOLTAGE_UV+ULTRA_LOW_VOL_STEP))) {
			__pm_stay_awake(chg->smooth_soc);
			val->intval = ++ui_soc;
			opti_uevt_v = smooth_soc = false;
			if (chg->batt_psy)
			...
			__pm_relax(chg->smooth_soc);
			break;
		} else if ((hvdcp_changed==2) && (ui_soc - val->intval) > 1) { //as. 52->50
			__pm_stay_awake(chg->smooth_soc);
			val->intval = --ui_soc;
			opti_uevt_v = smooth_soc = false;
			if (chg->batt_psy)
		...
		if((!soc_inited)) {
			oplus_calibrate_ponsoc(&val->intval);
		} else {
			soc_inited = true;
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		...
		if (abs(((BATVOL - val->intval)/1000)) < 2) {
			opti_uevt_v = true;
		} else {
			opti_uevt_v = false;
			BATVOL = val->intval;
	...
	case POWER_SUPPLY_PROP_CHARGE_TERM_CURRENT:
		...
		if(val->intval != (-1 * ITERM_MA) ){
			smblite_lib_set_prop_batt_iterm(chg, (-1 * ITERM_MA));
		}
		...
		const union power_supply_propval *val)
{
	...
	switch (prop) {
	case POWER_SUPPLY_PROP_CHG_LIMIT:
		chg->chg_limit = val->intval;
		if (val->intval == 1) {//lcd on
			vote(chg->fcc_votable, DEFAULT_VOTER, true, oplus_limit_chg_i);
		} else {
			vote(chg->fcc_votable, DEFAULT_VOTER, false, 0);
			vote(chg->fv_votable, DEFAULT_VOTER, false, 0);
			if (val->intval == 2) {//sell mode
	...
	break;
	case POWER_SUPPLY_PROP_SET_CHG_CURRENT:
		chg->chg_current_limit = val->intval;
	break;
	case POWER_SUPPLY_PROP_DEMO_MODE:
		chg->demo_mode = val->intval;
	break;
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		if (val->intval > 0) {
	...
	case POWER_SUPPLY_PROP_AGING_CHG_CTL:
		...
		pr_info("set aging_chg_ctl %d,chg_sts only en->dis if TURE",(val->intval));
	break;
	case POWER_SUPPLY_PROP_STATUS:
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		if(chg->chg_limit == 2)
			chg->batt_profile_fv_uv = OPLUS_SELL_MAX_VOL;
		else
	...
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX:
		if(chg->chg_limit == 1)
			chg->batt_profile_fcc_ua = oplus_limit_chg_i;
		else
...
static int smblite_init_hw(struct smblite *chip)
{
	...
	//weak batt volt drops,soc is 50%
