...
{
	if (chg->chg_limit == 2)
		chg->concurrent_mode_supported = false;
				...
				union power_supply_propval *val)
{
	...
	rc = smblite_lib_read(chg, BATTERY_CHARGER_STATUS_1_REG(chg->base), &stat);
	if (rc < 0) {
		smblite_lib_err(chg, "Couldn't read BATTERY_CHARGER_STATUS_1_REG rc=%d\n",rc);
		return rc;
	}
	if (stat & BIT(5)) {
		val->intval = POWER_SUPPLY_HEALTH_SAFETY_TIMER_EXPIRE;
		smblite_lib_err(chg, "charge safety timer expired\n");
		goto done;
    }
...
static int smblite_lib_hvdcp3_force_max_vbus(struct smb_charger *chg)
{
	...
	pr_info("HVDCP3 : detected=%s, prev_vbus=%d, vbus_now=%d\n",
				     ...
				     union power_supply_propval *val)
{
	...
	if (rc < 0) {
		smblite_lib_err(chg, "Couldn't read USBIN I channel rc=%d\n", rc);
		return rc;
	}
...
static void smblite_lib_usb_plugin_locked(struct smb_charger *chg)
{
	...
	if (chg->chg_limit == 2) {
		rc = smblite_lib_masked_write(chg, USB_CMD_PULLDOWN_REG(chg->base),
									...
									vbus_rising ? 0 : EN_PULLDOWN_USB_IN_BIT);
		if (rc < 0)
			smblite_lib_err(chg, "Couldn't configure pulldown on USB_IN rc=%d", rc);
	}
					      ...
					      bool rising, bool qc_charger)
{
	...
	pr_info("IRQ: %s %s, hvdcp3_detected=%s\n", __func__, (rising ? "rising" : "falling"),
			(chg->hvdcp3_detected ? "True" : "False"));
}
...
static void smblite_lib_icl_change_work(struct work_struct *work)
{
	...
	if(charger_log_enable)
		pr_info("icl_settled=%d\n", settled_ua);
}

extern int ui_soc;
extern int batt_status;
extern int qbgsoc;
extern bool soc_updated;
extern bool limit_maxvol;
extern int BATVOL;
static void soc_work(struct work_struct *work)
{
	struct smb_charger *chg = container_of(work, struct smb_charger, soc_work.work);

	if ((batt_status == POWER_SUPPLY_STATUS_FULL) && (!limit_maxvol)) {
		/* soc++, if full, but soc is not 100 */
		if ((soc_updated) && (ui_soc < 100)) {
			ui_soc++;
			soc_updated = false;
			if(charger_log_enable)
				pr_err("soc_work full,++");
		}
		return;
	}

	if ((batt_status != POWER_SUPPLY_STATUS_CHARGING) && (ui_soc > 0)
		&& ((BATVOL <= UPPER_LIMIT_OF_ULTRA_LOW_VOLTAGE_UV) || (ui_soc > qbgsoc))) {
		if (soc_updated) {
			ui_soc--;
			power_supply_changed(chg->batt_psy);
			if(charger_log_enable)
...
int smblite_lib_init(struct smb_charger *chg)
{
	...
	INIT_DELAYED_WORK(&chg->soc_work, soc_work);
...
int smblite_lib_deinit(struct smb_charger *chg)
{
	switch (chg->mode) {
	case PARALLEL_MASTER:
		cancel_delayed_work_sync(&chg->soc_work);
