...
static int vnd_do_pending_cancel(struct geni_i2c_dev *gi2c)
{
	int timeout = 1;
			 ...
			 int num)
{
	...
	if ((geni_ios & 0x3) != 0x3) { //SCL:b'1, SDA:b'0
		GENI_SE_ERR(gi2c->ipcl, true, gi2c->dev,
			"IO lines in bad state, Power the slave 0x%x\n", (geni_ios & 0x3));
		if ((0x0 != (geni_ios & 0x3))) { //Only one of SCL or SDA occur error
			if (0 == vnd_do_pending_cancel(gi2c)) {
				gi2c->prev_cancel_pending = false;
			}
		}
	...
	for (i = 0; i < num; i++) {
		...
		if (!timeout) {
			...
			if ((geni_ios & 0x3) != 0x3) { //SCL:b'1, SDA:b'0
				GENI_SE_ERR(gi2c->ipcl, true, gi2c->dev,
					"%s: IO lines not in good state(%x), rtl_based(%x)\n",
