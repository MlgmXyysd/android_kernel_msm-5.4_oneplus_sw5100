...
void bcmsdh_send_uevent(void *info, uint reason)
{
	char event[] = "WLAN_FILTER_EVENT=bcm_wlan";
	char switch_event[] = "WLAN_SWITCH_EVENT=bcm_driver";
	char wifi_fail_reason[] = "WLAN_MONITER_REASON=0";
	char *envp[4];
	int ret;
	struct device *dev;
	bcmsdh_os_info_t *bcm = (bcmsdh_os_info_t *)info;
	if (reason > 9 || bcm == NULL || bcm->dev == NULL) {
		pr_err("[dhd]%s:prereason: %d\n", __func__, reason);
		return;
	}
	pr_info("[dhd]%s, start send %d\n", __func__, reason);
	dev = (struct device *)bcm->dev;
	wifi_fail_reason[strlen(wifi_fail_reason)-1] = reason + '0';
	envp[0] = (char *)&event;
	envp[1] = (char *)&switch_event;
	envp[2] = (char *)&wifi_fail_reason;
	envp[3] = 0;
	ret = kobject_uevent_env(&(dev->kobj), KOBJ_CHANGE, envp);
	if (ret)
		pr_err("[dhd]%s:reason: %d\n", __func__, reason);
}
