...
#define SSID_MASK_LEN 10

static inline char * OPLUS_SSID_MASK(const char *ssid)
{
	size_t i = 0;
	size_t len = 0;
    static char ssid_mask_txt[SSID_MASK_LEN];
    if (ssid == NULL) {
        ssid_mask_txt[0] = '\0';
        return ssid_mask_txt;
    }

    len = strlen(ssid);
    if (len > 4) {
        ssid_mask_txt[i++] = ssid[0];
        ssid_mask_txt[i++] = ssid[1];
        ssid_mask_txt[i++] = '*';
        ssid_mask_txt[i++] = '*';
        ssid_mask_txt[i++] = '*';
        ssid_mask_txt[i++] = ssid[len - 2];
        ssid_mask_txt[i++] = ssid[len - 1];
        ssid_mask_txt[i++] = '\0';
    } else if (len < 3) {
        ssid_mask_txt[i++] = ssid[0];
        ssid_mask_txt[i++] = '*';
...
wl_conn_debug_info(struct bcm_cfg80211 *cfg, struct net_device *dev, wlcfg_assoc_info_t *info)
{
	...
	rssi = wl_ext_get_rssi(cfg, info->bssid);
	wl_ext_get_sec(dev, 0, sec_info, sizeof(sec_info), TRUE);
	WL_MSG(dev->name, "Connecting with " OPLUSMACDBG " ssid \"%s\", len (%d), "
		"channel=%d, sec=%s, rssi=%d\n", OPLUSMAC2STRDBG((u8*)(&info->bssid)),
		OPLUS_SSID_MASK(info->ssid), info->ssid_len, wf_chspec_ctlchan(chanspec), sec_info, rssi);
	...
	SUPP_LOG(("[%s] Connecting with " OPLUSMACDBG " ssid \"%s\",chan_cnt:%d\n",
		dev->name, OPLUSMAC2STRDBG((u8*)(&info->bssid)),
		...
		wlcfg_assoc_info_t *info)
{
	...
	wl_ext_get_sec(dev, 0, sec_info, sizeof(sec_info), TRUE);
	WL_MSG(dev->name, "Reconnecting with " MACDBG " sec=%s\n",
		MAC2STRDBG((u8*)(&reassoc_params.bssid)), sec_info);
	...
	struct cfg80211_connect_params *sme)
{
	...
	if (assoc_info.reassoc) {
		// terence 20200530, should call wl_ext_iapsta_update_channel() to move AP channel?
...
wl_handle_link_down(struct bcm_cfg80211 *cfg, wl_assoc_status_t *as)
{
	...
	BCM_REFERENCE(dhdp);
#endif /* BCMDONGLEHOST */
	WL_MSG(ndev->name, "Link down Reason: %s\n", bcmevent_get_name(as->event_type));
	...
	const wl_event_msg_t *e, void *data)
{
	...
	WL_MSG(ndev->name, "%pM(ch:%3d/%sMHz) => %pM(ch:%3d/%sMHz)\n",
