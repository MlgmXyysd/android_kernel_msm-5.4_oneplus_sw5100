...
#define OPLUS_DEFAULT_SCAN_TIME -1
static int oplus_scan_active_time = OPLUS_DEFAULT_SCAN_TIME;
static int oplus_scan_passive_time = OPLUS_DEFAULT_SCAN_TIME;
static bool oplus_set_scan_time = false;
void oplus_set_custom_scan_time(void)
{
	oplus_set_scan_time = true;
	oplus_scan_active_time = CUSTOM_PRODUCT_ACTIVE_TIME;
	oplus_scan_passive_time = CUSTOM_PRODUCT_PASSIVE_TIME;
	pr_warn("dhd %s exit active %d, passive %d\n", __func__, oplus_scan_active_time, oplus_scan_passive_time);
}

void oplus_reset_custom_scan_time(void)
{
	pr_warn("dhd %s enter\n", __func__);
	oplus_scan_active_time = OPLUS_DEFAULT_SCAN_TIME;
	oplus_scan_passive_time = OPLUS_DEFAULT_SCAN_TIME;
	oplus_set_scan_time = false;
}
#endif
...
#ifdef CUSTOM_PRODUCT
		params_v2->active_time = htod32(oplus_scan_active_time);
		params_v2->passive_time = htod32(oplus_scan_passive_time);
#else
...
#ifdef CUSTOM_PRODUCT
		params->active_time = htod32(oplus_scan_active_time);
		params->passive_time = htod32(oplus_scan_passive_time);
#else
...
#ifdef CUSTOM_PRODUCT
	if (oplus_set_scan_time) {
		oplus_reset_custom_scan_time();
	}
#endif
...
//		return 0;
	...
	if (request) {		/* scan bss */
		...
		if (p2p_ssid) {
			if (cfg->p2p_supported && cfg->p2p_wdev) {
				/* p2p scan trigger */
				if (p2p_on(cfg) == false) {
...
#ifdef WL_SCHED_SCAN
#define PNO_TIME                    (10*50)//30  modify pno time to 500s
#define PNO_REPEAT                  4
