...
#ifdef CUSTOM_PRODUCT
extern int dhd_get_factory_mac(char *buf, int len);
#endif /* CUSTOM_PRODUCT */
...
MODULE_IMPORT_NS(BCM_WLAN_EXPORT_ONLY);
...
#if defined(OEM_ANDROID)
#ifdef BUILD_USER_TEST
uint dhd_console_ms = 20;
#else
...
extern int dhd_get_mem_sleep_current(void);
#endif
...
static int dhd_pm_callback(struct notifier_block *nfb, unsigned long action, void *ignored)
{
	...
	if (dhd->busstate != DHD_BUS_DOWN) {
		printf("%s: action=%ld, suspend=%d, suspend_mode=%d\n",
			__FUNCTION__, action, suspend, conf->suspend_mode);
	}
#ifdef CONFIG_DEEPSLEEP
	if ((dhd_get_mem_sleep_current() == PM_SUSPEND_MEM) && (dhd->busstate != DHD_BUS_DOWN)) {
		pr_err("%s: forbid PM_SUSPEND_MEM %d\n", __func__, dhd->busstate);
		return -ERESTARTSYS;
	}
#endif
	if (suspend) {
...
#ifdef TPUT_MONITOR
		wl_tput_monitor_set_timer(dhd, 0);
#endif /* TPUT_MONITOR */
...
_dhd_set_mac_address(dhd_info_t *dhd, int ifidx, uint8 *addr, bool skip_stop)
{
	...
	if (ret < 0) {
		DHD_ERROR(("%s: set cur_etheraddr "OPLUSMACDBG" failed ret=%d\n",
			dhd_ifname(&dhd->pub, ifidx), OPLUSMAC2STRDBG(addr), ret));
		goto exit;
	} else {
		memcpy(dhd->iflist[ifidx]->net->dev_addr, addr, ETHER_ADDR_LEN);
		if (ifidx == 0)
			memcpy(dhd->pub.mac.octet, addr, ETHER_ADDR_LEN);
		WL_MSG(dhd_ifname(&dhd->pub, ifidx), "MACID "OPLUSMACDBG" is overwritten\n", OPLUSMAC2STRDBG(addr));
	}
...
dhd_set_mac_address(struct net_device *dev, void *addr)
{
	...
	WL_MSG(dev->name, "macaddr = "OPLUSMACDBG"\n", OPLUSMAC2STRDBG(dhdif->mac_addr));
#ifdef WL_CFG80211
...
#ifdef CUSTOM_PRODUCT
extern void oplus_set_custom_scan_time(void);
#endif
...
#ifdef CUSTOM_PRODUCT
	if (!ret) {
		oplus_set_custom_scan_time();
	}
#endif

	WL_MSG(net->name, "Exit ret=%d\n", ret);
	if (!ret)
		need_debug_print_clk = 0;
	return ret;
}
...
#ifdef DBG_PKT_MON
	/*copy_to_user might sleep, use mutex instead of spin*/
...
#ifdef CUSTOM_PRODUCT
extern char custom_firmware_path[64];
extern char custom_nvram_path[64];
static inline char* dhd_get_custom_nv_path(void)
{
	if (strlen(custom_nvram_path) > strlen(CUSTOM_PRODUCT_NV_DIR)) {
		return custom_nvram_path;
	} else {
		return CUSTOM_PRODUCT_NV_DIR CUSTOM_PRODUCT_DEFAULT_NV;
	}
}
#endif
...
#ifdef GET_CUSTOM_MAC_ENABLE
	ret = wifi_platform_get_mac_addr(dhd->info->adapter, ea_addr.octet, iface_name);
	if (!ret) {
...
#ifdef CUSTOM_PRODUCT
	/* let the factoty device MAC address override default device MAC*/
	ret = dhd_get_factory_mac((char *)&buf, sizeof(buf));
	if (ret)
		DHD_ERROR(("%s: can't get custom address\n", __FUNCTION__));
#endif /* CUSTOM_PRODUCT */

	DHD_ERROR(("%s: use %s generated mac_address "OPLUSMACDBG"\n",
		__FUNCTION__, ret ? "firmware" : "custom", OPLUSMAC2STRDBG(&buf)));
...
#ifdef CUSTOM_PRODUCT
	DHD_ERROR(("Firmware up: op_mode=0x%04x, MAC="OPLUSMACDBG"\n",
		dhd->op_mode, OPLUSMAC2STRDBG(dhd->mac.octet)));
#else
	DHD_ERROR(("Firmware up: op_mode=0x%04x, MAC="MACDBG"\n",
		dhd->op_mode, MAC2STRDBG(dhd->mac.octet)));
#endif
