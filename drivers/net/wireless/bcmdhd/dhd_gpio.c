...
#define NVRAM_PROPNAME            "nvram"
#define FW_PROPNAME               "firmware"
#define TWOONLY_PROPNAME           "oplus,2g_only"
#endif
#endif

#ifdef CUSTOM_PRODUCT
char custom_firmware_path[64] = CUSTOM_PRODUCT_NV_DIR;
char custom_nvram_path[64] = CUSTOM_PRODUCT_NV_DIR;
bool custom_2g_only;
#endif
static int
...
static int dhd_wlan_set_carddetect(int present)
{
	...
	if (present) {
		...
		sdhci_msm_card_detect(1, 0, true);
...
#ifdef CUSTOMER_HW_PLATFORM
		sdhci_msm_card_detect(1,0, false);
...
#endif
#ifdef CUSTOM_PRODUCT
		rc = of_property_read_string_index(root_node, NVRAM_PROPNAME, 0, &nv_path);
		if (rc || !nv_path) {
			printf("%s: get custom nvram path fail\n", __FUNCTION__);
		} else {
			strncat(custom_nvram_path, nv_path, strlen(nv_path));
			printf("%s: get custom nvram path ok %s\n", __FUNCTION__, custom_nvram_path);
		}
		custom_2g_only = of_property_read_bool(root_node, TWOONLY_PROPNAME);
		printf("%s: get 2g_only [%s]\n", __FUNCTION__, custom_2g_only ? "supported" : "unsupported");
#endif
	} else
#endif
	{
