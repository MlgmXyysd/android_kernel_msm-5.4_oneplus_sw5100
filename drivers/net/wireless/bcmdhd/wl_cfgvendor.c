...
// modify for compatibility
	...
	char *current_pos;
	u32 cmd_offset;
#endif /* (WL_ANDROID_PRIV_CMD_OVER_NL80211 && OEM_ANDROID) || defined(WL_PRIV_CMD_OVER_NL80211) */
// MOG-OFF: OEM_ANDROID
...
static int wl_cfgvendor_lstats_get_info(struct wiphy *wiphy,
	struct wireless_dev *wdev, const void  *data, int len)
{
	// modify some variables to 'static' to shrink too big stack size
	static char iovar_buf[WLC_IOCTL_MAXLEN];
	struct bcm_cfg80211 *cfg = wiphy_priv(wiphy);
	int err = 0, ret = 0, i;
	wifi_radio_stat *radio;
	static wifi_radio_stat_h radio_h;
	...
	static wlc_rev_info_t revinfo;
	wl_if_stats_t *if_stats = NULL;
	wl_if_infra_enh_stats_v2_t *if_infra_enh_stats = NULL;
	dhd_pub_t *dhdp = (dhd_pub_t *)(cfg->pub);
	static wl_pwrstats_query_t scan_query;
	wl_pwrstats_t *pwrstats;
	static wl_pwr_scan_stats_t scan_stats;
	int scan_time_len;
	uint32 tot_pno_dur = 0;
	static wifi_channel_stat cur_channel_stat;
	cca_congest_channel_req_t *cca_result;
	static cca_congest_channel_req_t cca_req;
	...
	if (!IS_PRIMARY_NDEV(cfg, wdev_to_ndev(wdev))) {
	...
	if (wl_cfg80211_determine_p2p_rsdb_scc_mode(cfg)) {
	...
	if (FW_SUPPORTED(dhdp, ifst)) {
		err = wl_cfg80211_if_infra_enh_ifstats_counters(bcmcfg_to_prmry_ndev(cfg),
				if_infra_enh_stats);
		if (!err) {
...
#ifdef LINKSTAT_EXT_SUPPORT
	err = wldev_iovar_getbuf(bcmcfg_to_prmry_ndev(cfg), "bssload_report", NULL,
...
const struct nla_policy vendor_acs_attr_policy[BRCM_VENDOR_ATTR_ACS_LAST] = {
	[BRCM_VENDOR_ATTR_ACS_CHANNEL_INVALID] = { .type = NLA_UNSPEC },
	[BRCM_VENDOR_ATTR_ACS_PRIMARY_FREQ] = { .type = NLA_U16 },
	[BRCM_VENDOR_ATTR_ACS_SECONDARY_FREQ] = { .type = NLA_U16 },
	...
	[BRCM_VENDOR_ATTR_ACS_CH_LIST] = { .type = NLA_NUL_STRING },
	[BRCM_VENDOR_ATTR_ACS_FREQ_LIST] = { .type = NLA_NUL_STRING },
...
static const struct  nl80211_vendor_cmd_info wl_vendor_events [] = {
		...
		{OUI_GOOGLE, GOOGLE_NAN_EVENT_MATCH_EXPIRY}, //45
		...
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC},
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC},
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC}, //50
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC},
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC},
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC},
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC},
		...
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC},
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC},
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC}, //60
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC},
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC},
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC},
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC},
		...
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC},
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC},
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC}, //70
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC},
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC},
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC},
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC},
		...
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC},
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC},
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC}, //80
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC},
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC},
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC},
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC},
		...
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC},
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC},
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC}, //90
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC},
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC},
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC},
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC},
		...
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC},
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC},
		{OUI_BRCM, BRCM_VENDOR_EVENT_UNSPEC}, //100
		// SYNA specific
		{OUI_BRCM, SYNA_VENDOR_EVENT_ROAMING} //101
};
...
wl_cfgvendor_apply_cmd_policy(struct wiphy *wiphy)
{
	...
	for (i = 0; i < n_cmds; i++) {
		...
		if (wl_vendor_cmds[i].info.subcmd == DEBUG_SET_HAL_START) {
			pr_err("wl_cfgvendor 0x%x, %u", wl_vendor_cmds[i].info.vendor_id, (unsigned int)wl_vendor_cmds[i].doit);
		}
	}
}
#endif /* LINUX VER >= 5.3 */
