...
#if defined(WL_CFG80211) || defined(TPUT_MONITOR)
bool
...
#endif /* WL_CFG80211 || TPUT_MONITOR */
	...
	u32 channel)
{
	...
	if (cur_if) {
		...
		if (cur_if->ifmode == ISTA_MODE) {
			if (conf->war & SET_CHAN_INCONN && conf->chip == BCM43752_CHIP_ID) {
				chanspec_t fw_chspec;
...
static void
wl_tput_monitor_mod_timer(struct wl_apsta_params *apsta_params, uint timeout)
{
...
void
wl_tput_monitor_set_timer(dhd_pub_t *dhd, uint timeout)
{
	struct wl_apsta_params *apsta_params = (struct wl_apsta_params *)dhd->iapsta_params;
	wl_tput_monitor_mod_timer(apsta_params, timeout);
}
...
wl_tput_dump(struct wl_apsta_params *apsta_params, struct wl_if_info *cur_if)
{
	...
	if (sta == NULL || (sta->ver != WL_STA_VER_4 && sta->ver != WL_STA_VER_5)) {
		WL_MSG(cur_if->dev->name,
			"rssi=%s, tx=%3d Mbps(rate:%4d%2s), rx=%3d Mbps, "\
			"tput_sum=%3d Mbps\n",
			rssi_buf, cur_if->tput_tx, rate/2, (rate & 1) ? ".5" : "",
			cur_if->tput_rx, apsta_params->tput_sum);
	} else {
		WL_MSG(cur_if->dev->name,
			"rssi=%s, tx=%3d Mbps(rate:%4d%2s), rx=%3d Mbps(rate:%4d.%d), "\
			"tput_sum=%3d Mbps\n",
			rssi_buf, cur_if->tput_tx, rate/2, (rate & 1) ? ".5" : "",
			cur_if->tput_rx,
			dtoh32(sta->rx_rate)/1000, ((dtoh32(sta->rx_rate)/100)%10),
			apsta_params->tput_sum);
	}
	...
	struct wl_if_info *cur_if, const wl_event_msg_t *e, void *data)
{
	...
	if (etype == WLC_E_RESERVED && reason == ISAM_RC_TPUT_MONITOR) {
		...
		apsta_params->tput_sum = tput_sum;
		for (i=0; i<MAX_IF_NUM; i++) {
			if (monitor_if[i]) {
				tmp_if = &apsta_params->if_info[i];
		...
		if (etype == WLC_E_LINK) {
			if (flags & WLC_EVENT_MSG_LINK) {
				...
				wl_tput_monitor_mod_timer(apsta_params, 0);
			}
		}
	}
		...
		if (event_type == WLC_E_LINK) {
			if (!(flags & WLC_EVENT_MSG_LINK)) {
				WL_MSG(cur_if->ifname,
					"[%c] Link down with "OPLUSMACDBG", %s(%d), reason %d\n",
					cur_if->prefix, OPLUSMAC2STRDBG(&e->addr), bcmevent_get_name(event_type),
					event_type, reason);
				...
				WL_MSG(cur_if->ifname, "[%c] Link UP with "OPLUSMACDBG"\n",
					cur_if->prefix, OPLUSMAC2STRDBG(&e->addr));
			...
			WL_MSG(cur_if->ifname, "[%c] Link down with "OPLUSMACDBG", %s(%d), reason %d\n",
				cur_if->prefix, OPLUSMAC2STRDBG(&e->addr), bcmevent_get_name(event_type),
				event_type, reason);
#if defined(WLMESH) && defined(WL_ESCAN)
			if (mesh_if && apsta_params->macs)
