...
enum andr_vendor_subcmd {
	...
	WIFI_SUBCMD_CELL_AVOID_SUBCMD_SET_CONFIG = ANDROID_NL80211_SUBCMD_CHAVOID_RANGE_START,
...
enum rtt_attributes {
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 3, 0)) || defined(CUSTOMER_HW4)
...
enum wifi_roaming_status_attr {
	BRCM_ROAMING_STATUS_ATTR_TYPE,
	BRCM_ROAMING_STATUS_ATTR_REASON,
	BRCM_ROAMING_STATUS_ATTR_STATUS,
	BRCM_ROAMING_STATUS_ATTR_CHANNEL,
	BRCM_ROAMING_STATUS_ATTR_BSSID
};

enum debug_attributes {
// from linux 5.3.0, there is extra policy to check the attr
// type which forbidden the enumerate value to be 0(invalid)
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 3, 0)) || defined(CUSTOMER_HW4)
...
typedef enum wl_vendor_event {
	...
	// SYNA specific
	SYNA_VENDOR_EVENT_UNSPEC		= 100,
	SYNA_VENDOR_EVENT_ROAMING		= 101,
...
// modify for compatibility
	...
	((LINUX_VERSION_CODE > KERNEL_VERSION(3, 14, 0)) || defined(WL_VENDOR_EXT_SUPPORT))
...
#define SUPP_LOG(x)
#define SUPP_EVENT(x)
#endif /* WL_SUPP_EVENT && (kernel > (3, 14, 0)) || WL_VENDOR_EXT_SUPPORT */
