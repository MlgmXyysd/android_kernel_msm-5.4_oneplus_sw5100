...
#define CUSTOM_MAX_KSO_ATTEMPTS (3000)
...
void read_hardware_register_host_wake(void)
{
	void __iomem *vir_adress = ioremap(0x0056F000, 32);

	pr_err("#==================================================\n");
	pr_err("[DHD]#TLMM_GPIO_CFG : %08X \n", readl_relaxed(vir_adress));
	pr_err("[DHD]#TLMM_GPIO_IN_OUT : %08X \n", readl_relaxed(vir_adress + 4));
	pr_err("[DHD]#TLMM_GPIO_INTR_CFG : %08X \n", readl_relaxed(vir_adress + 8));
	pr_err("[DHD]#TLMM_GPIO_INTR_STATUS: %08X \n", readl_relaxed(vir_adress + 12));
	pr_err("[DHD]#TLMM_GPIO_ID_STATUS: %08X \n", readl_relaxed(vir_adress + 16));
	pr_err("[DHD]#TLMM_GPIO_LP_CFG: %08X \n", readl_relaxed(vir_adress + 20));
	pr_err("#==================================================\n");

	iounmap(vir_adress);
}

extern void bcmsdh_send_uevent(void *info, uint reason);
static void report_illegal_interrupt(struct dhd_bus *bus)
{
	if (bus == NULL || bus->sdh == NULL || bus->sdh->os_cxt == NULL) {
		pr_err("[dhd]%s\n", __func__);
		return;
	}
	bcmsdh_send_uevent(bus->sdh->os_cxt, 1);
}
...
#ifdef DHD_DEBUG
			uint32 status, retry = 0;
			read_hardware_register_host_wake();
			R_SDREG(status, &bus->regs->intstatus, retry);
			DHD_ERROR(("%s: resumed on timeout, INT status=0x%08X\n",
				__FUNCTION__, status));
			report_illegal_interrupt(bus);
#else
