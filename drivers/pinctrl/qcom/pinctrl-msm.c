...
#include <../../oplus/dap_swd/dap_swd.h>
...
int msm_gpio_get_reg(struct gpio_desc *desc, struct msm_gpio_reg *reg)
{
	...
	struct msm_pinctrl *pctrl = gpiochip_get_data(chip);//container_of(chip, struct msm_pinctrl, chip);
	const struct msm_pingroup *g = &pctrl->soc->groups[gpio_chip_hwgpio(desc)];
	reg->io_reg = pctrl->regs[g->tile] + g->io_reg;
	reg->ctl_reg = pctrl->regs[g->tile] + g->ctl_reg;
	return 0;
}
