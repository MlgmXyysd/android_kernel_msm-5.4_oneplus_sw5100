...
static int __init qcom_proxy_of_init(void)
{
	...
	if (ext_boot_with_console()
		&& (strcmp(qcom_proxy_of_consumer_match[0].compatible, "qcom,msm-geni-console") == 0)) {
		strcpy((char*)qcom_proxy_of_consumer_match[0].compatible, "needn't match");
			pr_err("console needn't match");
	}
