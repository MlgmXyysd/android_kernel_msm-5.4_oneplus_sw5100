...
static int qti_pmic_lpm_suspend_late(struct device *dev)
{
	...
	if (mem_sleep_current == PM_SUSPEND_MEM) {
		dev_err(dev, "deep sleep handle suspend_late()\n");
...
static int qti_pmic_lpm_resume_early(struct device *dev)
{
	...
	/* mem_sleep_current = PM_SUSPEND_MEM in DeepSleep */
	if (mem_sleep_current == PM_SUSPEND_MEM) {
		u16 addr = 0x8740; //DS_CUT_PMIC_ADDR
		unsigned int val = 0;
		rc = regmap_bulk_read(chip->regmap, addr, &val, 2);
		if (rc < 0) {
			dev_info(dev, "Register read failed, addr=0x%04X, rc=%d\n", addr, rc);
		} else {
			dev_err(dev, "DS_CUT, A_0x%04X,V_0x%04x\n", addr, (u16)val);
		}
		dev_err(dev, "deep sleep handle resume_early()\n");
