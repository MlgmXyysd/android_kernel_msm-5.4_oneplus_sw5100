				       ...
				       const char *page, size_t count)
{
	...
	int permissive = strncmp(current->comm, "recovery", 8);

	if (ret < 0)
		return ret;

	if ((permissive != 0) && (max_discard & (q->limits.discard_granularity - 1)))
		return -EINVAL;
	...
	if ((permissive != 0) && (max_discard > q->limits.max_hw_discard_sectors))
